...
已加入 AD 網域、已登入 Windows 的使用者，經前端 Nginx（SPNEGO/Kerberos）驗證後，由 Nginx 傳入帳號標頭（預設 `X-Remote-User`）即自動登入；非網域電腦或未帶標頭者，一律回到 `/login` 用既有方式登入。屬**附加**登入方式，不取代既有後端，`jtdt-admin` 仍可從 `/login` 做 break-glass；auditor 即使經 proxy 認人仍強制 2FA。使用者查詢/同步沿用既有 LDAP / AD 連線。

設定在 `/admin/sso` 頁最下方「Reverse Proxy SSO」。**只採信直連來源 IP 在信任清單內的請求（不看 X-Forwarded-For），並建議服務只綁 127.0.0.1、Nginx 一律覆寫標頭**，防止標頭偽造。完整部署（AD service account / setspn / ktpass / Nginx / 瀏覽器設定 / 資安防護）見 `reverse_proxy_sso.md`。
