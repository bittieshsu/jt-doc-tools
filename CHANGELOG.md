...
### Windows：VC++ 執行階段的修復不必重新開機

* v1.16.23 修復被換成舊版的 VC++ 執行階段時，走的是 Microsoft 的 vc_redist 安裝程式；
  但同一次開機裡它只要回過一次「需要重新開機」，之後就不肯再動作，安裝只能請你重新開機再跑 `jtdt update`。
  現在改成直接修復系統裡已安裝的 VC++ 執行階段套件：不必重新開機、也不用重新下載，
  修不好才退回 vc_redist。安裝程式、指令列安裝與 `jtdt update` 都一樣。

## [1.16.23] - 2026-09-25

### Windows：裝了 OxOffice 之後，OCR 的中文辨識變差（EasyOCR 載不起來）

* OxOffice 11.0.5 的安裝檔會把系統裡的 Visual C++ 執行階段換成它內含的舊版（14.29）。辨識引擎 EasyOCR
  需要 14.40 以上，於是 OCR 一律改用 Tesseract，中文辨識差很多，畫面上只寫「選用 本機 EasyOCR 失敗 → 退回
  本機 Tesseract」。原本的檢查只看登錄檔（仍然寫著新版），判定「已是最新」而沒有修。
  用 v1.16.20 ~ v1.16.22 的安裝程式裝上 OxOffice 的機器，以及自己另外裝了 OxOffice 11.0.5 的機器都會遇到。
* 安裝程式與 `jtdt update` 現在會檢查 System32 裡實際的檔案版本，被換成舊版時自動修復（不需要重新開機）。
  如果同一次開機裡已經修過一次，會提示重新開機後再執行 `jtdt update`。
* 「相依套件」頁的 EasyOCR 那一列會標出這個狀況。
* `jtdt update` 會分開講「EasyOCR 沒裝」與「裝了但載不起來」（原本一律寫「未裝」）。
* 已經遇到的：以系統管理員身分執行一次 `jtdt update` 即可修好。

## [1.16.22] - 2026-09-25

### Windows 安裝程式：OxOffice 在慢網路上裝不起來、裝好了又被當成失敗

* OxOffice 的安裝檔約 400 MB，放在 GitHub 上。有些網路連到那裡很慢，下載可能不完整卻沒有報錯，
  接著安裝回 1625「系統原則禁止這項安裝」—— 看起來像權限問題，其實是檔案沒下載完整。
  現在下載完會比對檔案大小與數位簽章，不對就重新下載（最多三次），三次都不行才改裝 LibreOffice。
* 安裝 OxOffice 回 3010（「已安裝，建議重新開機」）原本被當成失敗，接著又多裝一套 LibreOffice。
  現在算成功（轉檔不需要重新開機）。
* OxOffice 的安裝記錄寫到 `%ProgramData%\jt-doc-tools\Logs\oxoffice-msi.log`，失敗時查得到原因。
* 以上要用 v1.16.22 的安裝程式才有；指令列安裝（`install.ps1`）一併修正。

### OCR：兩件同時第一次辨識，下載的模型會壞掉

* 第一次使用 OCR 要先下載辨識模型（約 300 MB）。這時如果又送了第二件（等太久又按了一次、或一次拉進兩個檔），
  兩件會一起把模型下載到同一個暫存檔、互相蓋掉，解壓時出現 `Bad CRC-32`：模型白下載，
  那一件還改用 Tesseract 辨識（中文準確度差很多）。現在同一時間只會有一件在下載，其他的等它下載完直接使用。
* 模型下載失敗時，原本每一頁都會重新下載一次；現在 10 分鐘內不再重試（這段時間改用 Tesseract），之後自動再試。
* 透過 API 送出的 OCR 作業，完成訊息也會寫出實際用了哪個辨識引擎、有沒有改用 Tesseract（原本只有網頁那條會寫）。
* 服務記錄：每完成一件作業就多出約 50 行 `Registered tool`，真正的錯誤被擠到看不到；現在只在啟動時記一次。

## [1.16.21] - 2026-09-24

### Windows 安裝程式：升級時沿用原本的「區域網路存取」設定

* v1.16.20 的安裝檔把「區域網路存取」改成預設不勾之後，原本開著的機器重跑安裝程式升級，
  會被改成只有本機連得到 —— 給整個單位共用的機器，其他人會突然連不進去。
  現在升級時會先讀現有的服務設定：原本開著就預先勾起來（元件頁上看得到，也可以取消）。
* 升級時也保留原本的監聽位址（例如只綁在某一張網卡上）與自訂的 port；原本升級會被改回 8765。
* 全新安裝仍然預設不勾。
* 已經用 v1.16.20 的安裝檔升級、變成只有本機連得到的：重跑一次 v1.16.21 的安裝程式，
  在元件頁把「區域網路存取」勾起來即可（資料會保留）。

## [1.16.20] - 2026-09-24

### Windows：全新安裝後逐一跑工具，修掉幾個只在 Windows 上發生的問題

用安裝程式在 Windows 上全新安裝，再把各工具實際跑一遍：

* **PDF 轉文書檔很慢、右邊的轉換後預覽是空的**：產生預覽時自己組的 Office 設定檔路徑在 Windows 上不是合法網址，
  每次都要等到 180 秒逾時，一頁的 PDF 要轉 3 分鐘。現在改走共用的轉檔流程，秒級完成、預覽正常。
* **「jtdt-reform」引擎在 Windows 上交不出 Word 檔**：同一個原因讓 ODT 轉 Word 那一步逾時失敗；
  失敗後改交的 ODT 檔案卻被命名成 `.docx`、畫面寫「完成」，用 Word 打開會說檔案毀損。
  現在轉換正常；萬一真的改交 ODT，檔名會是 `.odt`，完成訊息也會說明。
* **擷取文字的 ODT 輸出**在 Windows 上可能卡住：同樣改走共用的轉檔流程。
* **查 Office 版本會在背景留下程式**：Windows 上查 OxOffice / LibreOffice 版本時啟動的程式不會自己結束，
  每查一次就多留一個。現在直接讀程式檔案裡的版本資訊，不再啟動它。
* **OCR 第一次使用**要先下載辨識模型（約 300 MB），畫面原本只寫「準備中」，網路慢時看起來像當掉。
  現在會說明正在下載、只需要一次。
* 「設定 → 應用程式」裡顯示的版本改成實際安裝的版本（原本顯示安裝檔打包當時的版本）。
* 安裝時下載相依套件那一步的完整輸出，現在會寫進 `%ProgramData%\jt-doc-tools\Logs\setup-python-sync.log`，
  安裝失敗時查得到原因。
* 指令列安裝（`install.ps1`）挑 OxOffice 安裝檔的規則寫錯，一直比對不到而改裝 LibreOffice，已修正。

### Windows 安裝程式（v1.16.20 的安裝檔）：最後一步偶爾當掉、「區域網路存取」改成預設不勾

* 安裝程式偶爾在全部裝好之後當掉，約四次中一次：服務其實已經在執行，
  但「設定 → 應用程式」沒有登記、開始功能表也沒有捷徑。
  原因是安裝程式用的 NSIS 元件在輸出很多時會弄壞記憶體（NSIS 官方的已知問題，修正版尚未發佈）。
  已改用不收輸出的呼叫方式，詳細過程照樣寫在 `%ProgramData%\jt-doc-tools\Logs\installer.log`。
  **這一項與 OxOffice 自動安裝的修正包在安裝檔裡，要用 v1.16.20 以後的安裝檔才有**；
  用舊安裝檔遇到的話，服務照常可用，網址是 `http://127.0.0.1:8765/`。
* 「區域網路存取」改成**預設不勾**。勾了會讓同一個網路上的其他電腦也能連進來，而自己一台電腦使用時沒有登入。
  要給整個單位共用的，安裝時把它勾起來；已經裝好的，重跑一次安裝程式勾選即可（資料會保留）。

### API 手冊：三處照抄會拿到錯的東西

* 插入頁碼的總頁數要寫 `{N}`，手冊原本寫 `{total}`，照抄會把字面的「{total}」印在每一頁上。
  現在兩種寫法都認得。
* OCR 回的是作業編號（背景處理），不是 PDF。
* PDF 轉圖片要分兩步：先送出轉換、再用回傳的 `upload_id` 下載。

## [1.16.19] - 2026-09-24

### 會議摘要：版面主題移到下載按鈕上面；匯出文件的表格更好讀

* 會議摘要結果區的「版面主題」原本是下載按鈕那一排的最後一格，按鈕一多就被擠到下一行、跑到按鈕下方。
  現在在按鈕上方自成一行，先選版面再按下載。
* 匯出成 PDF、Word、ODF 時，表格的表頭底色原本只包住文字、沒有填滿整格，框線也偏重。
  現在表頭整格上色、框線改用主題的淡色、內容列隔行加淡底色，列高也不再撐得很高。
  「Markdown 轉辦公文件」用的是同一套版面，一起受惠。

## [1.16.18] - 2026-09-24

### 左側選單的捲軸可以按住拖動

左側選單內容比視窗長時，右緣會出現一條捲軸，但那條只是畫上去的樣子：滑鼠按下去拖不動，
反而會開始選取底下的文字，一拖就把整片選單反白。

* 現在可以按住捲軸上下拖動，拖動的距離會換算成選單實際要捲的距離。
* 可以按的範圍比看得到的那條寬一點，比較好按中；滑鼠移上去或拖動時捲軸會變亮。
* 拖動時不會再選到選單裡的文字。

## [1.16.17] - 2026-09-24

### LLM：接 Ollama 以外的服務也能用，API 金鑰改為加密存放

LLM 加值功能一直是照 Ollama 寫的。接其他 OpenAI 相容服務（vLLM、LM Studio、雲端服務）時有幾個地方會出錯，
其中兩個出錯的方式很難發現：

* **表單自動填寫的 LLM 逐欄校驗只打 Ollama 自己的端點，也不帶 API 金鑰。** 接其他服務時每一欄都問不到，
  而「問不到」被當成「沒問題」，結果顯示整份都填對了。現在 Ollama 以外的服務改走標準的 OpenAI 相容端點，
  並帶上設定的金鑰；一個回答都沒拿到時會直接顯示錯誤，部分欄位問不到時也會講出有幾個沒校驗到。
* **每一次請求都帶著 Ollama 專屬的欄位**（用來關掉模型思考的 `think`、`reasoning_effort` 等）。
  不接受未知參數的服務會直接退回。現在連線前會先確認對方是不是 Ollama，只有 Ollama 才會收到這些欄位。
  在其他服務上跑會思考的模型時，回應可能會慢一些。
* 串流回應的 `data:` 後面沒有空白時也讀得到；對方在串流途中回報錯誤時，那次呼叫會失敗並把原因寫進服務記錄，
  不再當成空白回答。對方回 4xx / 5xx 時，回應內容也會寫進服務記錄。
* **LLM 的 API 金鑰改為加密存放**，跟 SSO、通知、語音服務的祕密一樣：設定頁與 API 都不會顯示金鑰本身，
  設定檔權限改為只有服務帳號讀得到。舊版存的明文金鑰會在第一次讀取時自動搬成加密的。
  設定備份匯出 / 匯入時會換成新機器的金鑰。
* 「測試連線」時，存著的金鑰只會送給存檔時的那個位址。

## [1.16.16] - 2026-09-24

### 會議錄音轉逐字稿：選了台語辨識模式也送得出去

語音服務的「台語轉錄」辨識模式不做發言者分離，而我們每一件都會要求發言者分離 ——
管理員在設定頁把辨識模式換成台語之後，**每一件**送出去都會被退回。

* 送件前先查語音服務提供的辨識模式清單，只要求這個模式做得到的處理；台語模式不再要求發言者分離，
  「會發言的人數」也不會送出。
* 結果頁會寫出「這次用的辨識模式不做發言者分離」，不讓沒有發言者的逐字稿看起來像是分離失敗。
* 設定頁選到不做發言者分離的辨識模式時，下方會直接標出來。
* 讀不到辨識模式清單時照原本的設定送出，不自行猜測；萬一還是被退回，訊息會說明是辨識模式做不到，並建議重送或換模式。
* API 回應多一個 `diarize_skipped` 欄位。

## [1.16.15] - 2026-09-24

### 會議錄音轉逐字稿：語言可以選「韓文」

語音服務（JTLW）從這一版的對接起支援韓文，轉逐字稿的語言選單多了「韓文」。
原本選「自動判斷」時，韓文錄音其實已經辨識得出來；現在可以直接指定。

* 透過 API 呼叫時，`language` 可以送 `ko`；`ko-KR` 這種帶地區的寫法會收成 `ko` 再送出（跟 `en-US`、`ja-JP` 一樣）。
* 管理員若把處理設定改成「台語逐字稿」，那個設定不支援韓文，送件時會直接說明，請改選「自動判斷」或別的語言。
* 自動判斷一樣只看錄音開頭約 30 秒的講話；已知整場是韓文時，建議直接選「韓文」。

## [1.16.14] - 2026-09-23

### 文件翻譯、PDF 壓縮：完成後只留一顆下載鈕

這兩支工具完成時，上面的進度列會出現「下載」，下面的結果區又放了一顆下載鈕，
而兩顆拿到的是同一個檔、同一個檔名，看起來像兩種不同的東西。

* 拿掉結果區那一顆，下載一律走上面「下載 / 存至工作區 / 處理新檔案」那一排，跟其他工具的位置一致。
* 文件翻譯在預覽停下來的地方那一顆「下載翻譯後的檔案（共 N 頁）」保留：那顆只在整份頁數超過預覽時出現，
  是為了避免把「預覽只有前幾頁」誤會成「只翻到前幾頁」。
* 加了一條自動檢查：用共用進度列的工具，如果結果區自己又放了下載鈕，就要把進度列那一顆藏起來，
  之後新增的工具不會再出現兩顆。
...
jt-live-whisper 的縮寫原本在畫面與文件上寫成小寫 `jtlw`，跟 `JTDT` 放在一起看起來像是兩種不同的東西。
設定頁、轉逐字稿頁、錯誤訊息、介紹站、API 手冊、README 與疑難排解頁的**文字**一律改成 `JTLW`。

* **網址、模組名、設定檔名與頁內錨點維持小寫**（例如 `/admin/jtlw`）—— 那些改了會讓既有的書籤、
  API 呼叫與設定匯入對不上。
* 英文與日文介面一起改；並加了一條自動檢查，擋住之後又把縮寫寫回小寫（識別字不算）。

### 介紹站：手機選單裡的 GitHub 按鈕上緣像被切掉一截

畫面變窄、導覽收成選單之後，GitHub 那一格的上緣是方角、上框線又跟其他三邊不一樣，
看起來像被切掉一截。原本的樣式想做「上面一條分隔線」，但按鈕本身帶著四邊的框，兩者疊在一起就長成這樣。

* 現在跟上面的語言選單是同一種框：一樣寬、一樣高、一樣的圓角與框線，文字垂直置中。
* 中文、英文、日文三個版本都用瀏覽器在手機寬度打開選單量過，並加進既有的導覽列檢查
  （同寬、同高、同圓角、上下框線一致）。

## [1.16.12] - 2026-09-23

### 更新時不再印 uv 的「`UV_NATIVE_TLS` 已棄用」警告

為了讓 uv 在企業 TLS 檢查環境下改用作業系統的信任庫，原本同時設了新舊兩個環境變數
（`UV_SYSTEM_CERTS` 與 `UV_NATIVE_TLS`），以為 uv 會忽略不認得的那個。
但新版 uv 認得舊的那個，**每次都印一行「已棄用」警告**，出現在每一次 `jtdt update`
與安裝的輸出裡，讀起來像升級出了錯。

* 現在先問 uv 自己支不支援新的旗標，**只設它認得的那一個**：新版 uv 設 `UV_SYSTEM_CERTS`、
  舊版設 `UV_NATIVE_TLS`；問不到就用舊的（舊版一定認得）。自己設過其中一個的照樣尊重。
* `jtdt update`、Linux / macOS 安裝腳本、Windows 安裝時執行的 `setup-python.cmd` 三處一起改；
  Windows 那一段在實機的 `cmd.exe` 上驗過新版 uv、舊版 uv、使用者自己設定三種情況。
* 更新到這一版的那一次仍可能看到一次這行警告（跑更新的是舊版程式），下一次起就不會。
* 這個功能從 v1.12.12 加入以來一條測試都沒有，這次補上（含拿真的 uv 跑一次確認沒有警告）。

## [1.16.11] - 2026-09-23

### `jtdt update` 結尾一律印「Health check failed」—— 服務其實是好的

從 v1.15.11 起，**Linux、Windows、macOS 的 `jtdt update` 最後一步都會判成失敗**，
而服務早就正常起來了。原因是健康檢查用到的下載模組沒有真的 import 進來
（那一行被插進檔案裡一段產生出來的腳本字串），每一次連線都丟錯、又被當成「連不上」吞掉。

* 修正之後健康檢查會真的去問服務。
* **更新到 v1.16.11 的那一次仍會看到這段訊息**：跑檢查的是更新前那一版的程式。
  用瀏覽器打得開就代表更新成功，下一次更新起就正常。
* Windows「程式和功能」裡的版本號是在健康檢查通過後才更新的，所以這段期間一直停在舊版號；
  下一次更新成功時會自動補上。
* 同一個原因也讓幾件事在需要時做不起來，但都只印一行警告、不影響更新：
  Windows 自動補裝 VC++ 執行階段、下載繁中 OCR 語言檔的舊路徑、下載 WinSW 的備援路徑。

### 健康檢查失敗時，看得到服務真正的日誌

* **Windows**：原本去 `<資料目錄>\logs\jt-doc-tools.log` 找 —— 那個目錄從來不存在，
  於是永遠印「no log at …」。服務的日誌其實在
  `C:\ProgramData\jt-doc-tools\Logs\`：`jtdt-svc.err.log`（啟動訊息與例外堆疊）、
  `jtdt-svc.out.log`（服務自己的記錄）、`jtdt-svc.wrapper.log`（服務包裝程式）。
* **macOS**：原本只讀 `~/Library/Logs/jt-doc-tools.log`（服務自己的記錄），
  但 uvicorn 的啟動訊息與沒被攔下的例外堆疊寫在同目錄的 **`jt-doc-tools.err`** ——
  服務起不來時最需要的正好是那一個。現在兩個都印，`.err` 先。
* **Linux** 走 `journalctl`，原本就是對的，沒動。
* `jtdt logs` 用同一份清單；Windows 上用系統字碼頁（繁中是 cp950）寫的中文也讀得正確，
  而且只讀檔尾，日誌很大時不會整份載進來。
* 等服務起來的時間從約 15 秒放寬到最多 2 分鐘，每 15 秒印一次「還在等」。
  更新剛換過相依時，第一次啟動要重新編譯、防毒也會逐檔掃，15 秒很容易不夠 ——
  實測一台 Apple M2 筆電從很舊的版本更新上來，健康檢查開始時服務還在載入工具。
* 更新結尾那段 `DeprecationWarning`（`locale.getdefaultlocale`）拿掉了；
  Windows 改問系統的顯示語言來決定疑難排解頁連中文還是英文版。
* 疑難排解頁的健康檢查那一則補上這個情況與三個平台的日誌位置（中英日）。

### 同一種錯另外四處：用到從來沒定義過的名稱

用自動檢查掃全部程式碼，同一種錯（名稱沒有 import，平常走不到、出錯時才走到）還有：

* **瀏覽器遇到「需要登入 / 沒有存取權限 / 找不到頁面」時，友善的錯誤頁變成 500**（v1.14.16 起）。
* 匯入資產備份時，偵測到 zip 炸彈該回 400，變成 500（v1.15.13 起）。
* 統編資料庫排程填了不合法的值該回 400，變成 500（v1.14.51 起）。
* 切換 OCR 語言檔版本失敗時，該回「失敗」卻丟出例外（v1.7.5 起）。

新增自動檢查：`app/` 與 `tools/` 的每一支程式都不可以用到沒定義過的名稱
（自己寫的範圍分析，不依賴額外套件）。把任何一個修正拿掉，那一支就會紅。

### LLM 停用時：預設反灰，另外勾「停用時一併隱藏」才隱藏

* LLM 設定頁新增「停用時一併隱藏」。**預設不勾** —— 停用時工具與選項照樣看得到、反灰，
  看得出為什麼不能用；勾了才從畫面上拿掉。LLM 啟用時勾著也不影響任何東西。
* 只靠 LLM 的三支工具（逐句翻譯、文件翻譯、會議摘要）停用時在側欄與首頁**反灰並說明原因**
  （原本照常顯示、點進去才告訴你不能用）；勾了隱藏就從側欄、首頁與搜尋拿掉。
* 表單自動填寫與擷取文字的 LLM 選項原本一停用就整段不見，現在跟其他工具一樣反灰。
* 設定頁原本寫「關閉時 AI 加值會自動隱藏」，實際一直是反灰 —— 說明改成事實。
  工具數原本三個地方三個數字（10 / 12 / 13），現在從程式算出來。
* 首頁的工具清單改用跟側欄同一份資料：原本沒設定語音服務時，側欄反灰、首頁卻照常顯示。

### 其他

* 側欄搜尋的計數提示、權限矩陣的「內建帳號權限固定不可修改」、用印逐頁縮圖的頁碼提示，
  在英文與日文介面一直是中文（滑鼠移上去才看得到）。三處都改走翻譯，
  原本的翻譯檢查只認「等號後面直接是字串」，三元運算整條漏掉 —— 範圍一併放寬。
* 翻譯鍵的檢查原本只看 `{{ tr('…') }}`，漏掉 `tr('…')|replace(…)` 這種寫法；一併放寬。
* 用詞：「守門」改成「檢查」（台灣不用「守門」指自動測試），並加進用詞檢查。

## [1.16.10] - 2026-09-23

### 會議錄音轉逐字稿：排隊看得出來了，排隊時間也不再算進等待上限

語音服務 2026-09-23 升級之後，排在 GPU 隊伍裡時會回報前面還有幾件，辨識與校正的進度也會往前。

* 排隊時畫面寫「排隊中（前面還有 N 件）」（件數包含別人的作業）；校正時寫「已完成 N / M 批」。
* **排隊的時間不算進等待上限**：上限只從輪到之後開始算（15 分鐘與錄音長度的一半取大）。
  排在好幾場長會議後面也不會被誤判逾時；原本多給的 60 分鐘排隊寬限拿掉。
* 排隊本身有總上限（4 小時）：語音服務的 GPU 伺服器掛住時，這一件不會無限期佔著名額。
* **不拿「進度多久沒動」當取消條件**：語音服務說明輪到之後排隊標記會晚幾秒才消失、
  校正只在每批做完時更新、長會議的間隔沒量過 —— 用停多久來判斷卡住，會誤殺長會議。
* 還沒升級的語音服務（回應裡沒有排隊欄位）維持原本的做法。
* 順帶修正：「對方佇列滿了，N 秒後再試」「等了 N 分鐘…」這幾句伺服器送出的訊息，
  英文與日文介面一直顯示中文（語系檔裡沒有對應的譯文），現在都有了，並收進翻譯檢查。

### 會議錄音轉逐字稿：「自動判斷」只看錄音開頭

語音服務量過：「自動判斷」**不是逐段判斷**，只看錄音開頭約 30 秒的講話決定整場用哪一種語言。
中文為主、但開頭有人先用英文講話的會議（20 秒就夠），會整場用英文辨識，中文句的錯誤率從 15% 變成 88%。
**開頭的靜音、雜音、沒有人聲的音樂不影響**（判斷前會先濾掉沒人講話的部分），不需要剪掉。
指定語言不會比自動判斷差（猜對時兩者一模一樣）。

* 語言選項的說明改成講出這件事；v1.16.9 寫的「辨識會更準」拿掉 —— 真正的差別是「不會整場猜錯」。
* 中英句子交錯的會議：選佔多數的語言，另一種語言的整句大多無法正確辨識；中文句子裡夾英文術語沒有問題。
* 疑難排解頁新增「中文會議卻整場辨識成英文」一則，排隊那一則依新的行為整段重寫。

### 會議錄音轉逐字稿：游標在波形上時，標籤一起顯示那一刻是誰在講

時間標籤旁多了發言者（跟逐字稿同色的圓點與名字，改過名字的顯示新名字）。
兩段之間沒人講話的空檔只顯示時間，不猜一個人上去；兩人重疊時顯示後開始講的那一位。

### 會議摘要：改了發言者名字，其他區塊跟著換

原本只有逐字稿換成新名字，待辦卡片還寫「負責：S1」、摘要與「誰講了多少」也還是舊的。

* 卡片（負責人與內文）、摘要、章節、心智圖、「誰講了多少」都會跟著換，下載的檔案也是。
* 自由文字裡只換**代號形狀**的名字（`S1`、`SPEAKER_00`）；真人名字只換整個欄位等於它的地方
  （把「王」改名不會連「王道」一起換），`S12` 也不會被 `S1` 換掉一截。
* 「誰講了多少」依改名後的逐字稿重算：只改某一段時，那一段的次數與時間跟著移過去；
  兩個代號改成同一個名字會合併成一位。
* 只改某一段時，卡片與摘要裡的代號不動 —— 那裡的 `S1` 指的是整個人。

### 從「會議錄音轉逐字稿」送到「會議摘要」：改過的名字會帶過去

在轉逐字稿那邊改過的發言者名字，送到會議摘要之後原本又變回 `S1`；現在照改過的顯示
（包括只改某一段的）。送過來時畫面也會直接捲到「開始分析」。

### 會議摘要：進度條在「寫摘要」時就已經滿了

原本四個階段各佔四分之一，而且把「開始第 N 件」算成「做完第 N 件」—— 寫摘要一開始就是 100%。
複審與「事件與影響」那一整輪（呼叫次數跟擷取重點一樣多）也沒有回報進度，進度條會在同一格停很久。

* 改成照「還要呼叫幾次模型」分配，每一段都有回報，**完成之前不會到 100%**。
* 英文與日文介面的進度文字原本一直是中文，現在有譯文。

### 會議摘要：上傳後的預覽明確標出「只列出前幾段」

原本是一行灰色小字，容易以為整份逐字稿只讀到這幾段。現在是醒目的說明框，
寫出「整份共 M 段，這裡只列出前 N 段 —— 分析會用整份逐字稿」，清單最後一列再提示還有幾段沒列出。

### 會議錄音轉逐字稿：同步 API 被語音服務退回時回 500

`POST /tools/meeting-transcribe/api/meeting-transcribe` 在語音服務退回請求時一律回 500，
而 API 手冊寫的是 400。500 會讓呼叫端以為服務壞了而一直重試。現在照原因回：
參數被退回 400、語音服務處理失敗 502、沒設定或連不上 503、等太久 504。網頁那條路不受影響。
API 手冊一併寫清楚「自動判斷」只看錄音開頭。

### 掃描修正：預覽偶爾載不出來，或顯示的是舊的結果

同一頁的預覽原本固定寫同一個檔。拖曳四個角或轉向會連發好幾個請求，畫面會取消舊的，
但伺服器那邊照樣算完、晚一點寫回同一個檔 —— 最新那張被蓋成舊的（狀態列卻寫著新的），
或瀏覽器讀到寫到一半的檔，右邊一直空白。現在每一次預覽一個檔、寫完才換上，只留最新幾張。

---

## [1.16.9] - 2026-09-23

### 文件翻譯：Excel 存的試算表翻完之後，用 Excel 開是空白的

症狀是**預覽正常，下載的檔案用 Excel 開卻是空白**（或先跳出「內容有問題，要修復嗎」，
修復之後資料全不見）。

原因在寫回檔案那一步：我們用的 XML 函式庫輸出時會把**沒註冊過的命名空間前綴改名**
（`x14ac` 變成 `ns3`），並**丟掉內文沒用到的宣告**（`xr2`、`xr3`）。XML 本身仍然合法，
但 Excel 存檔時會在工作表開頭寫 `mc:Ignorable="x14ac xr xr2 xr3"` —— 那是**用前綴的名字**
指的，名字對不上宣告，Excel 就把整份內容判成毀損。

* **預覽看不出來**：預覽是 OxOffice / LibreOffice 畫的，它們不理這個屬性。
* **Word 與 PowerPoint 存的檔案是同一個病**：主文件同樣有 `mc:Ignorable`，
  文字方塊還有 `Requires="wps"`。
* **一直沒被測到**：我們的試算表樣本都是 OxOffice / LibreOffice 存的，裡面沒有這個屬性。
  新的測試素材刻意做成 Excel 與 Word 存出來的樣子，並拿所有真實樣本再跑一次；
  另外驗了同一個前綴在不同層指向兩個網址、以及兩件作業同時寫檔的情況。

現在寫回時前綴與宣告一律照原檔，`standalone="yes"` 也留著。
**已經翻好的檔案不會自動修好，請重新翻一次。**

「逐句翻譯」不收試算表，不受影響。

### 會議錄音轉逐字稿：選了「中文」送出就失敗

選項裡的語言選「中文」時，每一件都在送件當下被語音服務退回，畫面上是
`JTLW 回報：invalid_request（欄位 language）`。原因在本系統：下拉送出的是 `zh`，
語音服務認的是 BCP-47 的 `zh-Hant`（「英文」「日文」剛好送 `en` / `ja` 所以沒事）。
API 手冊一直寫的是 `zh-Hant`，只有頁面上的下拉寫錯。

* 下拉改送 `zh-Hant`；伺服器端另外把 `zh`、`zh-TW` 這類常見寫法換成 `zh-Hant`，
  照慣例送 `zh` 的 API 呼叫端也不會被退回。**簡體（`zh-CN`）不會被換成繁體。**
* 語音服務不收某個語言設定時，畫面改成講下一步：改選「自動判斷」再送一次。
* **測試為什麼一直是綠的**：測試用的假語音服務不檢查語言代碼。現在它跟正式 API
  一樣會退回不認得的代碼，下拉裡的每一個選項都會真的送一次。
* 語言下拉旁加上說明：已知整場使用的語言時，建議直接選取，辨識會更準。

### 會議錄音轉逐字稿：錄音最後很長一段沒有文字時會提示

語音服務那側告知：它的伺服器在傳結果途中重啟時，可能把只傳了一半的逐字稿當成功
回來（對方已修，2026-09-23 已上線）。一段都沒有的本系統本來就判失敗；**少了後面一截的
那種**，現在會在結果頁提示「錄音最後 N 分 N 秒沒有任何文字」。對方修好之後，這個提示
主要剩「錄音忘了停」的提醒作用；先留著，並把每一件的空白長度記下來，之後再決定留不留。

* **只提示，不判失敗**：錄音忘了停、結尾是掌聲或音樂時也會長這樣，而那是完整的
  逐字稿。提示直接寫出空白有多長，使用者自己知道錄音最後在做什麼。
* 門檻 60 秒：語音服務量了 25 場錄音的尾端空白，最長的是 41.8 秒。

### 介紹站

* 語音整合那一段改成「重要錄音不會離開你的網路」（README 同步）。

---

## [1.16.8] - 2026-09-23

### 語音服務失敗時的訊息：兩句說錯方向、一句沒給下一步

語音服務那側核對我們的疑難排解頁時指出兩處，**產品畫面上的錯誤訊息也是同一個錯**：

* **401 與 403 分開講**。原本合成一句「確認金鑰是不是被撤銷或打錯了」—— 但 403 是
  金鑰**對**、只是沒有這個動作的權限，照「打錯 / 撤銷」去查會查不出問題。現在 403 會說
  「請對方的管理員幫這把金鑰加上權限」，缺哪一個權限寫進服務記錄。
* **網址過期不是排隊造成的**。對方收到送件就立刻拉檔，之後排多久都不會再拉那個網址。
  原本寫「多半是排隊太久；請管理員延長錄音檔網址的有效期」—— 前半句讓人去懷疑對方的
  佇列，後半句叫管理員去改一個**根本不存在的設定**。
* **回報成功但一段逐字稿都沒有**：語音服務的伺服器在傳結果途中重啟時可能發生（對方已修、
  尚未全面上線）。本系統本來就判失敗、不會交出空的逐字稿；訊息現在會說「重新送一次通常就好」。

這幾句原本**一條測試都沒有**、也不在翻譯檢查的範圍內（寫在錯誤碼對照表外面）——
現在收進同一張表，兩邊都涵蓋，並補上行為測試（兩個方向的變異都驗過）。
疑難排解頁三語同步，並新增「回報成功，但一段逐字稿都沒有」一則。

---

## [1.16.7] - 2026-09-23

### 轉逐字稿：等待上限多留 60 分鐘給排隊

語音服務的 GPU 自 2026-09-23 起**一次只處理一件、其餘排隊**，而排隊時與辨識中，
對方回報的都是「處理中」、進度完全不動 —— 本系統分不出「在排隊」與「卡住了」。
前面剛好排著一場 3 小時的中文會議時，要等約 18～30 分鐘，超過原本 15 分鐘的下限：
**那一件會被判逾時，而且本系統會主動請對方取消**一件本來會成功的作業。

* 等待上限改成「15 分鐘與錄音長度的一半兩者取大，**再加 60 分鐘排隊寬限**」。
  代價是真的卡住的作業要晚一小時才會放棄 —— 晚發現只是等久一點，誤殺是整件白做。
* 語音服務之後會補「正在排隊、前面還有幾件」的訊號，到時候改成排隊不計入上限，
  這段寬限就拿掉。
* 補上**真的跑一次輪詢迴圈**的測試（假時鐘、假的對方一直回「處理中」且進度不動）。
  原本這個上限一條測試都沒有，改壞了不會有任何東西紅。
* 疑難排解頁的說明同步更新。

### CI：排程測試在「正式機相依版本」那個 job 紅

「公開樹不可以有內網位址」這條檢查掃的是整個目錄，而那個 job 用 `uv sync` 把虛擬環境
建在 repo 裡 —— 第三方套件原始碼裡的位址被當成我們外洩的。本機永遠測不出來（開發樹裡
沒有虛擬環境）。判準改成**只看被 git 追蹤的檔案**（那才是真正公開出去的東西），
沒有 `.git` 的開發樹才退回列目錄；在一份真的 clone 上兩個方向都驗過。

### 輸入格裡的範例文字改淡

瀏覽器預設的範例文字顏色跟欄位裡真正的文字差不多深，多行的範例（會議摘要的貼上框、
會議背景）看起來就像已經填好的內容。全站統一改成更淡的灰，並在真的瀏覽器裡量
「比瀏覽器預設淡」補了檢查（判準用亮度，不寫死色碼）。

> 檢查第一版是空過的：管理頁那一條用 `input[placeholder]` 找欄位，對到的是
> **側欄的搜尋框**（紫底白字、本來就很亮）—— 把全站那條規則拿掉照樣會過。
> 改成指定頁面裡真正的欄位之後才有牙齒。
...
### ⚠⚠「我的作業」寫 24 小時，結果其實 2 小時就被清掉

已完成的作業按「開啟」得到 **410 檔案已過期**，而作業清單上寫著「結果檔會在
24 小時後自動清除」。正式機上看：一件不到 5 小時前完成的會議摘要，作業紀錄還在、
狀態「完成」，但分析結果、逐字稿、歸屬紀錄全部已經被清掉。

原因是**兩個期限對不上**：全站的作業結果與「開啟」要讀的資料都放在暫存區
（保留 2 小時），而作業保留期是 24 小時。v1.14.31 修過一次（讓作業結果目錄改用
作業保留期），**但沒有任何一支工具把結果放在那個目錄** —— 那次只修了一半，
影響的是**全站所有背景作業**，不只會議摘要。

* 清理暫存區時，**還在作業保留期內（或還在跑）的作業**，它的結果檔、檔名帶它
  編號的暫存檔、歸屬紀錄，改跟著作業保留期走。
* 靠檔名裡的作業編號認人，**不靠每支工具自己登記** —— 那樣下一支新工具又會漏。
* 跟任何作業都無關的舊暫存檔照樣 2 小時就清（有反向對照的測試）。
* **清理原本有兩條路**：除了定期的保留期清理，還有一支每 30 分鐘跑的暫存清理，
  固定 2 小時、**連管理頁的暫存保留期設定都不看**。修正的第一版只改了前者，
  部署後在正式機上看，一件剛過 2 小時的作業檔案照樣被後者刪掉 —— 現在兩條都走
  同一段邏輯，管理頁的設定也真的生效了。
* 管理區「檔案保留 / 清理」頁的說明補上這一條。

> 已經被清掉的結果救不回來；這個修正從升級之後生效。

### 版面：數字欄位撐成整列寬、狀態被拆成兩行、JTLW 全名看不到

* 語音服務設定的「送出逾時」、工作區設定的三格、轉逐字稿的人數 —— 填一個小數字
  的欄位被撐成整列寬，單位字被推到最右邊。改成共用的固定寬度樣式（全站 5 處同一個
  形狀，一次改完）；「送出逾時」的說明從輸入框旁的裸文字移到下方說明行。
* 使用者清單的「● 啟用」在表格較擠時被拆成「啟 / 用」兩行 —— 之前的截圖都是空的
  使用者清單，所以一直沒被看到。只修這一欄的話，擠的空間會換到隔壁：角色徽章
  「法務資安」變成「法務資 / 安」—— 兩個一起改成不斷行。
* **JTLW 的全名（jt-live-whisper）與專案說明連結**：設定頁原本放在標題列右上角、
  很難看到，轉逐字稿工具頁則完全沒有。兩頁改成接在說明那一行後面，共用同一個元件。

> 這幾件都在**真的瀏覽器**裡量（欄位寬度、`getClientRects()` 的行數、可見的外框）
> 補了檢查。第一版的修法在管理頁**量起來仍是整列寬** —— 真正蓋掉寬度的是另一條
> 權重更高的規則，是這支檢查當場抓到的；它也抓到自己的一個空過（選項區在上傳前是
> 隱藏的，量到寬度 0）。

### ⚠⚠ 兩支會議工具的伺服器端標籤從上線起就沒翻過

用**有資料**的實例拍英文截圖時抓到：逐字稿排法的下拉、轉逐字稿的每一個階段、
心智圖上的節點類別、語音服務回報的每一種失敗原因 —— 在英文與日文介面下
**全部是中文**。共同點是「文字來自伺服器送來的資料」，樣板掃字面 `tr('…')`
的檢查一律看不到。

* 語系檔補 **66 條**（英文、日文各一份）。
* 五個來源補進「程式算出來的標籤」那一類檢查
  （`tests/test_i18n_dynamic_labels.py`）—— 逐個變異驗證過，
  每一個都只紅它自己那一條。
* 作業**失敗原因**也改成走 `tr()`：它跟作業訊息一樣是背景執行緒產生的中文，
  少包那一層的話，最需要讀懂的那一句永遠是中文。

> **報告只印前 4 條，於是後面的永遠沒有人看到。** 截圖工具的殘留中文清單
> 原本截斷顯示，「只有 4 條」看起來就像「只漏了 4 條」，而實際是 54 條
> （其中卡片標題那一整組被截掉了）。已改成逐條列出。

### 「語者」「講者」一律改成「發言者」，並補上檢查

2026-09-22 使用者當場糾正過一次，但**沒有任何東西擋著** —— 這一輪又在
三個地方發現漏網的（逐字稿排法的三個選項、轉逐字稿的階段文字、
匯出的發言統計表頭）。加進禁用詞表之後當場再抓到 4 處文件裡的。

> 判準要排掉**「母語者」**（那是正確的中文）。搜尋關鍵字刻意保留舊詞 ——
> 習慣用舊詞找工具的人不該因為我們改用詞就找不到。

### 貼上的逐字稿改用旗標判斷，不再比對檔名

貼上時前端塞的檔名**使用者看得到**（作業名稱、通知），所以它要照介面語言翻；
而伺服器原本拿那個檔名去比字串決定「要不要用它當標題」。翻掉之後那個比對
永遠不成立，標題會變成「Pasted transcript 會議記錄」，**而畫面上完全看不出
哪裡錯了**。改由 `pasted` 旗標決定，舊資料仍然靠檔名認得出來。

### 介紹站：語言選單跟 GitHub 按鈕不一樣高

兩個各自算自己的高度，實測差 **12.2 px**（下拉 30.2、按鈕 42.4），
而且垂直也沒對齊。

> **沒有去湊 padding 的數字** —— 換一種語言、改一次字級就又會歪掉。
> 用 `align-self: stretch` 讓下拉跟著那一列最高的那個走。
> 檢查 `tests/test_docs_nav_controls_line_up.py` 在真的瀏覽器裡量
> 三個語言版本的上下緣，變異驗證會精準報出那 12.2 px。

### 人數提示：從一場改成 20 場，而且**不改得更強**

原本引的是**一場**會議的數字。語音服務用 AISHELL-4 test 全部 20 場重量之後，
指定正確人數與不指定**分不出勝負**（13.24% vs 12.46%，配對 bootstrap 95%
信賴區間 [−2.72, +4.10] 跨過 0）。

**所以「不確定就留 0」維持原樣，但不可以改寫成「讓系統自己判比較好」。**

> 真正硬的是機制，不是那格數字：同一批 20 場裡，**最佳的群數從來沒有大於
> 實際人數**（10 場相等、10 場更少、0 場更多）。有 6 個人在場不代表聲學上
> 分得出 6 群，硬要分成 6 群時系統只能把講最多的那個人拆開。
>
> 也要誠實記下來：「發言量足以辨認」這個修飾語**那個語料驗不到**
> （20 場裡「標註的人數」與「≥1.6 秒段落裡出現過的人數」完全相同）。
> 留著它是因為上面那條機制說得通，不是因為量過了。

### 文件

* README 與介紹站新增**語音服務整合**一節（怎麼用 / 效果 / 為什麼這樣分工），
  英文與日文同步。
* 介紹站的「所見即所得」補上**會議摘要**的截圖與說明。
* 兩支新工具的說明精簡，並補上缺的圖示。
* `API.md` 收錄 `POST /tools/meeting-transcribe/api/meeting-transcribe`
  —— 原本的涵蓋檢查是從「有 `/api/` 的工具」建表的，**一個 API 都沒有的
  工具根本不會進表**，所以它看不見。判準改成跟註冊表比對。
...
* **播放鍵改成圓形的主要按鈕**（藍底白圖示、46 px）——
  原本跟旁邊那排次要按鈕長得一樣，是播放器的主要動作卻最不顯眼。
  播放的三角形視覺重心偏左，往右推 2 px 才看起來在正中間。
* **時間分主次**：現在播到哪裡是要一直看的數字（17 px、深色），
  總長度只是參考（13 px、淡色）—— 兩個一樣大一樣顏色的話，
  眼睛每次都要重新找。
* **游標在波形上移動時跟著一條線**，旁邊標出那個位置的時間 ——
  **點下去之前就知道會跳到哪裡**。靠近右邊時標籤自動翻到線的左邊，
  不然會被畫布切掉。
* 波形現在也畫出**目前播放位置**的細線（跟「已播過」的顏色分開）。

---

## [1.16.4] - 2026-09-22

### 「會議摘要」的逐字稿也可以改名字、也分色了

跟「會議錄音轉逐字稿」同一套做法、**同一組色票** ——
同一場會議在兩支工具之間換來換去時，`S1` 不應該一下子是藍的、一下子是綠的。

* 點發言者的名字就可以改，預設**同一位全部一起改**，
  勾勾拿掉就只改那一段。
* **統計也一起搬**：發言佔比那張圖是**以代號當鍵**的 ——
  不一起改的話，圖上還是 `S1` 而逐字稿已經是人名，
  同一個畫面上兩套名字。
* **`seq` 一個都不動** —— 決議與待辦的引用綁的是 `seq`，
  改名字不可以讓任何一條引用失效（檢查直接驗這一點）。

改完直接寫回存下來的那一份，不做另一層對照表：畫面、下載、心智圖、
發言佔比走的都是同一份資料，分兩個地方存一定會漂。
...
* **按下播放之後圖示要變暫停** —— 原本一直停在播放的三角形，
  看不出現在是不是在播。兩個圖示都畫出來、用外層的 `<span>` 切換
  （**不可以直接對 `<svg>` 設 `.hidden`**，`SVGElement` 沒有那個屬性，
  v1.15.44 踩過）。
* **波形太小**：一般會議錄音的峰值只有 0.1~0.3，照絕對值畫出來就是
  畫面中央一條細線。改成**依全曲最大值正規化**再開根號壓動態範圍
  （安靜的段落也看得見），取樣點 900 → 1200、高度 48 → 72 px。
* **點時間之後捲錯位置**：捲動用的是 `offsetTop`，而那是相對於
  「最近的**已定位**祖先」—— 逐字稿那一塊只有 `overflow:auto`、沒有
  `position`，所以量到的是「從面板頂端算起」，把上面的標題、波形、
  按鈕全都算進去了，於是固定偏掉那一段。
  改成兩個 `getBoundingClientRect()` 相減 ＋ 目前的 `scrollTop`，
  **不依賴任何 CSS**：之後誰把 `position` 加上或拿掉都不會再壞。

---

## [1.16.2] - 2026-09-22

### ⚠⚠ 波形與播放器**永遠不出現** —— `@router.get` 不會自動收 HEAD

畫面先送一個 HEAD 去問「錄音檔還在嗎」，還在才顯示播放器。
但 **FastAPI 的 `@router.get` 只註冊 GET**（Starlette 的 `Route` 會順便加 HEAD，
FastAPI 不會）—— 探測一律回 **405**，於是播放器與波形**一次都沒有出現過**，
而畫面上沒有任何錯誤。又一次「什麼都沒發生」型的無聲失敗。

兩邊都修：端點改成 `api_route(methods=["GET", "HEAD"])`（有檢查），
畫面**不再用探測請求決定要不要顯示** —— 直接掛上去，讓 `<audio>` 自己的
`error` 事件說話。那才是權威的訊號（涵蓋檔案被清掉與格式放不出來兩種），
而且少一次來回。

> **通則：不要用一個「問問看」的請求去決定功能出不出現。**
> 探測失敗與「真的沒有」在畫面上長得一模一樣，而探測本身多了一條會壞的路。
> 能讓真正要用的那個元件自己回報失敗時，就讓它報。
...
* **波形圖 ＋ 播放器**：點波形任何一處就從那裡播，點左邊的時間也一樣。
  波形在瀏覽器裡算（不必在伺服器上裝 ffmpeg）；
  **超過 60 MB 的錄音不解碼** —— 三小時的會議解開來是好幾 GB，
  分頁會直接掛掉。那時候退成一條**一樣可以點**的時間軸，播放完全不受影響。
* **播到哪裡，那一段就亮起來**並自動捲到畫面上。
* **每位發言者一個顏色**（文字色 ＋ 整列淡底色）——
  一整頁灰字看不出「這裡換人講了」。
* **點名字就可以改**：預設**同一位全部一起改**（一場會議裡同一個人被標幾百次，
  一段一段改沒有意義），旁邊的勾勾拿掉就只改那一段
  （辨識偶爾會把某一段掛錯人）。名字存回逐字稿本身 ——
  下載、複製、轉送「會議摘要」走的是同一份資料，分兩個地方存一定會漂。
* **存至工作區**：存的是**純文字**（帶 `[mm:ss]` 與發言者）。
  工作區收不下 `.json`，而純文字丟回「會議摘要」時時間軸還在。

> **播放走的是登入後的歸屬檢查，不是簽章網址。** 簽章那條是給對方的伺服器
> 拉檔用的（短效、免登入）；混用的話，要嘛把短效網址塞進頁面（會過期），
> 要嘛把播放網址做成永久公開（那就漏了）。

### 用詞：「語者」換成「發言者」

「語者」不是台灣的說法。連同「講者」（那在台灣偏向「演講的人」）一起
統一成**發言者** —— 同一件事用兩個詞，畫面上看起來就像兩件事。
搜尋關鍵字**新舊都留著**，習慣用舊詞找工具的人不受影響。
...
語音服務給了一份逐項的接入清單，對著它把缺的補上。

**⚠ 錯誤訊息只有一條路在用對照表。** 送件當下被拒絕走的是**例外**，
終態失敗走的是 `errors[]` —— 而人話的對照表只有後者在用。
對方 v1.7 起把「拉錄音檔」提前到送件當下，所以來源相關的錯誤現在多半走前者：
正式機上看到的是 `JTLW 拒絕了這個請求：source_not_allowed（欄位 source.url）`，
**而對照表裡明明就有「請管理員把位址加進允許清單」**。
措辭收進一處，兩條路共用，檢查驗「兩邊都委派過去、而且只有一份對照表」。

其他補上的：

| 項目 | 之前 | 現在 |
|---|---|---|
| 輪詢的牆鐘上限 | **沒有**（對方卡住就永遠輪詢下去） | 音訊長度 × 0.5、下限 15 分鐘，逾時會請對方取消 |
| 佇列滿了（`queue_full`） | 直接失敗 | 依對方回的 `retry_after_ms` 重送（同一把 Idempotency-Key） |
| 校正失敗（`partially_succeeded`） | 安靜地顯示原始辨識結果 | **畫面明講「你看到的是原始辨識結果」** |
| 沒要求的層（`task_not_requested`） | 當成錯誤 | 當成「那一層沒有」 |
| 校正統計 | 沒取 | 取 `/result`，顯示校正力道與改過 / 未改的段數 |
| `correction_level` | 沒送（由對方決定） | 送 `punctuation_only`（我們量過校正的傳播率，選最保守的） |
| Range 續傳 | 有（`FileResponse` 做的），但沒有檢查 | 加了行為測試：206 ＋ `Content-Range` ＋ 位元組要對 |

### ⚠⚠ 時間資訊掉在「交接」那一步，不是掉在辨識

逐字稿上看不到時間資訊。語音服務判斷是我們只取了校正後的那一層 ——
**核對之後不是**：正式機上存下來的 258 段**每一段都有**開始／結束時間與語者，
三層用 `seq` 對起來的那段一直是對的。

掉時間的地方在**交接**：

* **轉送「會議摘要」**時把逐字稿攤平成純文字 —— 純文字裡沒有時間，
  於是那邊的語者發言佔比與章節時間軸變成「這份逐字稿沒有時間戳記」。
  改成送 JSON（時間、語者、段號都留著）。
* **複製純文字**同樣丟掉時間。現在會帶行首的 `[mm:ss]` ——
  我們自己的逐字稿剖析器讀得懂那個格式，丟掉等於自廢武功。

> **對方的判斷聽起來很合理，但跟事實不合。** 存下來的檔案是決定性的證據 ——
> 一句 `sum(1 for x in segs if x.get("start_ms"))` 就分得出是「沒拿到」
> 還是「拿到了但沒傳下去」。**不要照著別人的診斷去改自己的程式。**

### 其他

- 「我的作業」按「**開啟**」回到這支工具時是一片空白 —— 那一頁根本沒有讀
  `?job=`。結果要靠 `upload_id` 才讀得回來，而它是上傳當下才有的，
  重新開一頁時要從作業的 `meta` 拿。
- `source_not_allowed` 現在**講得出是哪一個主機**被擋（`details.host`）——
  只說「請加進允許清單」的話，管理員還是得來回問要放行哪一個。

> **「錄音太長」那句話刻意不寫「6 小時」**：上限是對方的、在 `/capabilities`
> 裡。寫進我們的訊息就是把同一份事實抄第二份 —— 他們調整上限的那一天，
> 我們會很有自信地講一個錯的數字。檢查也跟著改成驗「說不說得出下一步」。

---

## [1.15.99] - 2026-09-22

### ⚠⚠ 按「開始轉逐字稿」完全沒反應 —— 進度區只放了一個空的 `<div>`

`JobProgress` 是靠進度區裡面的 `.job-reset` / `.job-bar-inner` / `.job-status`
接線的。只放 `<div id="mtJob"></div>` 的話：
...
「開始轉逐字稿」那顆按鈕從來沒有被接上事件。畫面上**沒有任何錯誤**：
上傳正常（那段在例外之前）、選項區正常展開，只有按鈕按了沒反應。
伺服器記錄佐證：`POST /upload` 有、`POST /start` **一次都沒有**。

**v1.15.36 的「文件擺正」踩過一模一樣的，而且已經寫進專案筆記。**

### ⚠⚠ 為什麼「每頁在真瀏覽器開一次」那一關沒抓到

那支工具**沒設定好時只渲染一句「請先去設定」**，連上傳區都不畫 ——
而掃描用的是全新的拋棄式實例，**永遠沒有設定**。
所以那一關從頭到尾掃的都是空殼，真正的介面**從來沒有在瀏覽器裡開過**。

兩道都補上：掃描前先把這類工具的設定種進去（另有一條驗「種子真的生效」，
不然哪天失效會靜靜地退回掃空殼），再加一支靜態檢查擋
「建了 `JobProgress` 卻沒有共用元件的標記」。兩個方向變異驗證過。

### 其他

- **辨識模式**：管理頁那一欄原本叫「處理設定」、畫面上只有一串代號
  （`meeting.balanced`），使用者連問兩次那是什麼。改名為「辨識模式」、
  顯示對方給的名稱與敘述（**他們的 `name` / `description` 是逐語言的物件
  不是字串** —— 原本直接 `str()` 會把整個 dict 的 repr 印到選單上），
  並說明它是什麼。
- **存好設定之後，側欄的工具立刻變成可用**：設定齊不齊翻轉時重新載入頁面
  （反灰是伺服器渲染的，在前端自己解鎖等於把那套判斷抄第二份）。
  存檔後也會重抓一次辨識模式清單 —— 原本會留著一句過期的「還沒設定」。
- **語言改用全站共用的下拉**，不用瀏覽器原生的那個。

---

## [1.15.98] - 2026-09-22

### ⚠ API 金鑰欄位：範例文字寫在只會誤導人的時機

`Authorization: Bearer <金鑰>` 這條標頭是**我們自己組的**，所以欄位裡只要金鑰。
但對方的文件寫的是整條標頭，照著複製很自然會把 `Bearer ` 一起貼進來。
...
現在存檔時會把前綴收掉（要求 `bearer` 後面真的有空白 —— 不然會把
以那幾個字母開頭的真金鑰吃掉一截，症狀同樣是 401）。

同時修掉範例文字本身：「留空＝不更動」原本是無條件的提示，
而它**只在沒存過金鑰時才會出現**（存過之後欄位有值、提示被蓋住）——
也就是它唯一現身的時機，正好是沒有東西可以「不更動」的時候。
改成存過才顯示那句，沒存過時顯示金鑰的樣子，並在下面講出
「只貼金鑰本身」與多貼前綴的後果。

### ⚠ 人數提示再收窄一層：「發言量足以辨認的人數」

v1.15.97 把「與會人數」改成「會發言的人數」。語音服務在**完整 37 分鐘**的
7 人中文會議上重測（先前那組數字量在 3 分鐘節錄上，他們已主動更正）：
...
**結論沒有翻轉**（指定正確人數仍然更差），但只看這張表會做出錯的決定：
那 7 個人裡有 3 位分別只講了 28.6 / 56.2 / 94.8 秒，
**聲紋不足以成群的人硬湊一群給他，代價是把主要講者拆散**。

而**不填也有代價，而且是更嚴重的那一種**：不指定時有
**4/7 位講者在輸出裡完全不存在**，他們說的每一句都掛在別人名下 ——
對引用機制來說那不是「少了幾秒」，是「有一條決議、標了一個錯的人」。

所以說明改成問「**發言量足以辨認**的人數」，並給一個估得出來的界線
（「只講一兩句的不要算進去」）。**不確定仍然留 0。**

### ⚠ 用詞：「運維」是大陸用法，台灣說「維運」

`OPS.md` 的標題與 README 的文件導覽那一列寫著「日常運維」。
**同一份產品裡本來就兩種寫法並存** —— `AUTH.md` 早就寫著「日常維運」、
兩支翻譯工具的文件領域清單也是「網路 / IT 維運文件」，
只有那兩處是反過來寫的（同「預估閱讀 / 閱讀時間估計」那一次）。
已進禁用詞表。

### 相依套件：anyio 升到 4.14.2（一個重大、兩個中低）

| 編號 | 等級 | 內容 |
|---|---|---|
| GHSA-82r6-8w77-94w6 | **重大** | `TLSStream` 用 IDNA 2003 編碼主機名稱，可能讓憑證比對被繞過 |
| GHSA-5p39-cfhj-2xmp | 中 | 行程池的 worker 在 stderr 沒被讀走時會永久阻塞 |
| GHSA-3w57-8xmc-8v26 | 高 | `run_process` 忽略 `extra_groups`，可能保留父行程的群組 |

三個都在 **4.14.2** 修好（我們原本是 4.13.0）。
anyio 是傳遞相依（`starlette` / `httpx` / `watchfiles`），我們沒有直接 import。

### 靜態掃描：三支檢查的正規式是指數級回溯

抓「引號裡的內容」的寫法 `(?:\\.|(?!\1).)*` 裡，兩個分支**重疊** ——
`\a` 既可以配成一次 `\\.`，也可以配成兩次 `.`。配不上時每種切法都會被試一遍：
...
三支都改掉，並加一條檢查擋住這個形狀再被複製一次。

同一輪另外兩處硬化：介紹站的語言下拉改成只接受「同目錄的 `.html` 檔名」
（值雖然是我們自己產生的，但那是 DOM 裡的字串，
限制成白名單形狀之後 `javascript:` 這條路就不存在了）；
舊工具網址的 308 轉址把路徑重新編碼一次。
...
會議錄音轉逐字稿原本問「與會人數」，說明還寫著「知道確切人數時填了會比較準」。
**那句話被量測推翻了。**

語音服務拿一場 7 人的中文會議實測：其中 4 位發言不到 10 秒（最少的只講了
1.5 秒，聲紋根本不足以辨識）。指定正確的 7 人之後**講者錯誤率反而變差** ——
強迫分成 7 群時，系統只能把主要講者的話切散來湊數。
系統自己判 5 人反而是合理的。

所以欄位改成問「**會發言的人數**」，並明講**不確定就留 0**
（系統自己判斷通常比錯誤的提示好）。

> **這一條用字面檢查釘住**：改回「與會人數」不會有任何測試變紅，
> 而結果會**系統性地變差**，使用者也不會知道為什麼 ——
> 跟 `OPS.md` 的 IIS 安裝順序同一類：只有照著做的人才會踩到。
>
> 也跟「不從請求的 Host 推算對外位址」是同一種判斷：
> **看起來理所當然的自動化，方向錯了就是系統性地變差。**
> 原本很可能會做成「從與會者名單自動帶入人數」—— 那會是負面的最佳化。

## [1.15.96] - 2026-09-21

### 語音服務設定頁：一輪修正

* **「音檔」不是台灣用法** —— 台灣說「錄音檔」或「音訊檔」。全站 39 處改掉，
  並進禁用詞表。判準要**排掉「錄音檔」** —— 只比對子字串的話，
  改對了反而會紅（同「在線 / 在線上」那一條踩過的）。
* **「處理設定」原本要自己打字**（`meeting.balanced` 這種代號）。
  管理員得記魔術字串，打錯還要等送件才失敗。改成**下拉，清單由對方提供** ——
  我們這邊不抄一份（抄了就會漂）。讀不到時保留目前的值並講出原因，
  **不會變成一個空的下拉**（那看起來像「沒有可選的」，而其實是連不上）。
* **「啟用」自己獨立一塊**：它決定整支工具反不反灰，跟下面那幾個「填什麼」的
  欄位不是同一個層級，擠在清單第一列看起來像另一個普通欄位。
* **標題右邊補上全名與專案說明連結**（`JTLW` = `jt-live-whisper`）——
  縮寫看不出自己在設定的是哪一套東西。
* **未加密的 http 會跳警示**：我們送出去的 API 金鑰在標頭裡、錄音檔內容也走
  同一條路。本機位址不提醒（那是安全來源例外，提醒只會變成雜訊）。

* **「對外位址」沒填時帶入你現在連進來的網址**，並講出它是從哪來的 ——
  **只帶進輸入框，送件時一律用存起來的值**。同一套服務有內網直連與反向代理
  兩種到達方式，照請求推算的話，從對外網域進來的人送出的作業會被對方的
  白名單擋掉，而症狀是「有些人可以、有些人不行」。
* 說明框漏掉版面用的樣式鉤子，看起來像「樣式沒套好」。

### 自簽憑證：**貼上憑證**，不是關掉驗證

內部服務常用自簽憑證。可以關掉驗證的開關留著（預設是驗），
但**正解是把對方的憑證貼進來** —— 只影響這一條連線，其餘不受影響。

存檔時會算出 **SHA-256 指紋**顯示在畫面上，**管理員要拿它跟對方公布的值
核對過再信任** —— 貼上一份沒核對過的憑證，等於把「我信任誰」這件事
交給貼上來的那個人。貼錯東西當下就擋（不然會在送件那一刻失敗，
而訊息是 ssl 的內部錯誤，看不出是設定頁貼壞了）。

> 關掉驗證的代價畫面上直接寫著：**任何人都可以冒充對方，
> 而我們每次送件都把 API 金鑰交出去。**
...
* `progress` **是物件不是數字**（裡面有 `percent` / `processed_audio_ms` /
  `total_audio_ms`）—— 當成數字的話 `isinstance` 永遠不成立，進度條不動。
* 階段在 `progress.stage`，**頂層根本沒有 `stage` 欄位**（頂層的只出現在
  錯誤物件裡，標示錯在哪一階段）。而且值域是**階段名**
  （`fetch` / `normalize` / `asr` / `diarization` / `correction` / `finalize`），
  不是我以為的任務名 —— 所以階段文字永遠顯示成預設的「處理中」。

兩個加起來的症狀是**「什麼都沒發生」**：作業其實正常跑著，畫面卻看不出來。

> **我自己的測試抓不到它**：那支假伺服器回的是我**猜的**形狀
> （`progress: 1.0` ＋ 頂層 `stage`），於是它跟我們的程式彼此一致、永遠全綠。
> 這跟上一版那個「拿自己算的期望值驗自己」是同一個家族 ——
> **假伺服器要照對方文件的契約寫，不是照我們以為的樣子寫。**
> 修正後假伺服器改成先回幾次「執行中」，才驗得到進度會不會動。

### 失敗訊息改成使用者看得懂的話

拿到對方的錯誤碼清單之後逐條翻譯，不再把代碼原樣丟出去
（`audio_too_long` → 「錄音太長，對方上限 6 小時，請先分段」）。

**其中一條特別容易誤導**：對方的「連不到來源」涵蓋**網址過期**，
而我們給的是短效簽章網址。照字面講「找不到檔案」的話，
使用者會去找一個其實還在的檔案 —— 所以 404 ＋ 短效網址一律講成
「網址已經過期，多半是排隊太久」。

## [1.15.94] - 2026-09-21

### 新工具：會議錄音轉逐字稿（工具總數 49 → 50）

音訊 / 視訊進去，**帶時間與語者**的逐字稿出來，轉好一鍵交給「會議摘要」。
辨識、語者分離與標點校正在外部語音服務那側跑（那幾件需要 GPU 與音訊上下文），
**摘要、決議、待辦、心智圖與翻譯全部留在本系統** —— 所以拆成兩支工具：
沒接語音服務的客戶，「會議摘要」照樣能用。

**沒在管理區設定好就反灰。** 這支依賴的是**外部服務**不是本機相依 ——
裝了什麼看不出來，只有設定填了沒有才知道。反灰的時候滑鼠移上去說得出原因與
該去哪裡設定，而頁面本身也直接講同一句話：**不可以是一個看起來正常、
按下去才失敗的上傳區**。

順帶把反灰的理由從樣板裡搬出來：原本只有「語言不符」一種原因，那句話
**直接寫死在兩個樣板裡**；加第二種原因時兩邊都會講錯話（同一份東西寫在
兩個地方一定會漂，本專案第 N 次）。現在理由由資料帶著。

### 這一版踩到的

* **`safe_remote_base_url()` 會把路徑丟掉** —— 那是它當 SSRF barrier 的方式
  （「呼叫端自己接固定路徑」）。第一版直接拿管理員填的整串當送件位址，
  於是請求打到 `/jobs` 而不是 `/api/v1/jobs`，對方回 404 而訊息只說「HTTP 404」。
  **只有真的送出去才看得到**：單元測試對得起來，因為那是我們自己算的字串。
  現在固定前綴由我們自己接，管理員照對方文件貼一整串也沒關係。
* **管理頁的檢查靠「路徑尾段」比對** —— `/admin/jtlw` 一次都沒出現在測試計畫裡，
  是尾段 `jtlw` 撞上的。實算：52 支管理頁有 **4 支**是這樣假通過的，其中兩支的
  完整路徑出現 0 次。同一個洞 v1.15.30 在 API 那條修過，管理頁這條當時漏掉。
  判準改成完整路徑，並補上那四支真正的驗收項。

### 會議摘要的「立場判定」取消

判錯「某人反對某案」不是品質問題，是會傷到人的錯誤，而讀的人沒有理由懷疑
（它跟決議卡長得一模一樣、也附著段號）。而且它**沒有可量的判準** ——
兩個人讀同一段話可能標出不同的立場。這支工具其他每一塊都是先有判準才有功能。

## [1.15.93] - 2026-09-21

### 短效簽章網址：讓外部服務取一個檔案，而不交出任何憑證

有些外部服務只收「網址」——**檔案不是我們上傳給對方**，是對方拿我們給的
網址自己來拉。

最順手的做法是發一把 API Token 給對方 —— **查過之後那條路不能走**：
`api_tokens` 沒有 scope 的概念，一把 token 解鎖全部 `/api/*`
（作業、通知、工作區、每一支工具）。只為了讓對方抓一個錄音檔而給出這些，
以本專案自己的標準不能接受。
...
* **到期時間要在簽章裡** —— 只簽檔案 id 的話，改 `exp` 就能無限延期。
* **驗不過一律 404，不是 403** —— 403 等於告訴對方「這個 id 是存在的」，
  而 id 本身就是我們不想外流的東西。格式不對 / 簽章錯 / 過期 / 檔案不在，
  四種在外面看起來要一模一樣。
* **網址用寫定的位址組，不可以拿請求的 Host** —— 同一套服務有三種到達方式
  （內網直連 / 反向代理的網域 / 測試機），照請求的 Host 組的話，
  從對外網址進來的人送出的作業會帶著對外網域、被對方的來源白名單擋掉，
  而症狀是「有些人可以、有些人不行」。檢查直接檢查那支函式的原始碼裡
  不可以出現 `request` / `headers` / `url.hostname`。

> 這條路**不需要動任何一道檢查**（實測過）：沒帶 bearer 的 GET 會落回 session，
> 而 `/api/` 在公開前綴裡；CSRF 只管不安全的方法。
> **POST 不是這樣** —— CSRF 是 pure-ASGI 中介層、跑在路由比對之前，
> 任何沒帶 token 的 POST 一律 403（連不存在的路徑也是）。
> 兩條路的檢查狀況完全不同，不能拿其中一條的結論套另一條。

### 會議摘要的「需 Office 引擎」扳手沒標

匯出成 PDF / Word / ODF 走的是 soffice，介紹站與 README 卻沒標扳手 ——
沒裝引擎的人會以為匯出壞了，而不是知道要裝什麼。
`.docx` / `.odt` 的**逐字稿輸入**不需要引擎（直接讀 zip 裡的 XML），
所以說明寫的是「匯出時需引擎」而不是照抄別支工具的「輸入時需引擎」。

### 推之前掃內網位址：規則有了檢查

四支評測腳本把推論機的位址寫成參數預設值，跟著同步進公開樹。
「推之前掃內網 IP」這條在慣例裡寫了很久，但**沒有檢查** ——
記了規則沒有檢查等於沒記，這次是推版前手動掃才發現的。

改成讀環境變數、預設 `localhost`，並加上
`tests/test_no_internal_addresses_in_public.py`：公開樹的**非測試檔**
不可以出現 RFC1918 的字面位址。測試檔排除在外（fixture 需要具體的位址當輸入，
那些是編出來的）；真正會出事的是預設值、設定範例與文件那一類
「照著跑就會連出去」的位址。

> **檢查裡不可以寫我們自己的位址** —— 那等於把它留在公開樹上。
> 判準因此放在「有沒有 RFC1918 的字面值」，示意值列進例外並寫理由，
> 例外清單自己也有一條檢查擋過期。
...
外部評語指出摘要講不出事故本身（發生了什麼、影響多大）。
**那不是模型的限制，是我們的設計**：摘要**只從已驗證的項目寫**、從頭到尾
看不到逐字稿，而「發生了什麼事」不屬於原本那四類，所以結構上不可能出現。

### ⚠⚠ 怎麼加，量出來差很多

| 做法 | 四類抓到率 | 四類捏造率 | 事件與影響 |
|---|---|---|---|
| 基準（四類） | 100% | **5%** | — |
| 五類擠同一次呼叫 | 88~100% | **16~19%** | 2 條（重複） |
| 五類＋補明確的分流規則 | 100% | **20%**（更糟） | — |
| 分開抽一輪 | 100% | 5~6% | 19 條（半數是進度回報） |
| **分開抽＋機械界線** | **100%** | **6%** | **7 條，逐條都對** |

**加規則 → 產出更多 → 噪音更多。** 看條目才知道原因：模型確實開始注意到
事實了，但**把事實歸進決議**（「rate limit 目前是每分鐘六十次」
「維護費對方開一年三十二萬」）。問題不是界線講不清楚，是**一次要判斷五類**本身。

所以「事件與影響」**自己走一輪**：獨立的提示、獨立的複審，
主路徑那四類的提示與規則**一字不差**。
**零退步是由構造保證的，不是調出來的。**

代價量出來了：抽取的呼叫數翻倍（160 分鐘的會議 29 → 61 次、147 → 303 秒）。
所以給了開關，而且**把取捨寫在畫面上**——事故與狀態會議建議開著，
純規劃的會議關掉可以少一半時間。

### 界線要機械可判，不能靠語感

第一版寫「個人的進度回報不算」——**模型分不出來**，因為「設定檔已經送出去了」
確實是已經發生的事。改成兩條機械的：

* 「做完了／送出去了／看過了」與「卡住了／還在等／還沒好」**一律不寫**。
* 每一條都要講得出**影響了什麼**或**一個具體的數字**，講不出來就不寫。

> **⚠ 中間我自己開了一個洞**：改寫規則時把「卡在權限沒開」那個例子
> **連同整段一起換掉**，結果那一類整批冒出來（4 條，還重複）。
> **不要為了修一個洞開另一個** —— 補回去時是**並存**不是取代。

> **檢查的第一版沒有牙齒**：`analyse` 裡有兩處逐類迴圈，
> 而我只驗「有出現 `MAIN_KINDS`」—— 只把其中一處改回 `KINDS` 照樣全綠。
> 判準改成「**一處都不可以用 `KINDS`**」才抓得到。

## [1.15.91] - 2026-09-20

### ⚠⚠ 心智圖的節點文字被硬砍，看起來像分析把內容弄丟了

節點標籤是 `文字[:60]` —— **砍 60 個字、不留任何記號**。畫面上看到的是
「…把環境變數（Environment Variable）加上 L」、「…全部 de」，
而**卡片與逐字稿其實都是完整的**。

使用者分不出這是「顯示時縮短了」還是「分析把後面弄丟了」，
而這兩件事的嚴重度天差地遠。

修法三件：**結尾加刪節號**（縮短要看得出來）、**不切在半個識別字中間**
（`LOG4J_FORMAT…`、`deployment.yaml`、`X-Forwarded-Proto` 都要算一個詞）、
**滑鼠移上去看得到完整內容**（原本提示框用的也是被截過的那一份）。

### ⚠ 上限 60 是拿評測語料定的，而那些語料根本碰不到它

翻出先前跑過的 **868 條**項目：中位數 **18** 字、**最長 56** ——
**一次都沒碰到 60**，所以截斷從來沒有出現在任何一次量測裡。
...
> **語料再多，形狀不對就照不到** —— 評測語料是合成的短會議，
> 真實會議的項目帶著英文術語與括號註解。這跟「合成樣本抓不到的那一類」同一條。

> **檢查的判準不可以跟被守的程式共用同一個定義**：我第一版用產品自己的
> `_is_token_char` 來驗「有沒有切在半個識別字中間」，於是把那個定義清空的變異
> **完全抓不到** —— 判準跟著變異一起移動，永遠自洽。測試改成自己寫一份。
>
> 同一條測試的第一版還用 `原文.startswith(留下來的那段)` 當判準，
> 那也是**沒有牙齒的**：`LOG4J` 正好是 `LOG4J_FORMAT…` 的前綴，
> 也就是**壞掉的那種切法會被判成合格**。

## [1.15.90] - 2026-09-20

### ⚠ 會議摘要的「發言時間」有一半是推估的，而畫面沒講

字幕檔（.vtt / .srt）與 JSON 的每一段都帶著**自己的結束時間** —— 那是量到的。
純文字逐字稿只有「誰在幾點幾分開始講」，結束時間是拿**下一段的開始**補上去的，
所以算出來的發言時間**把中間的停頓也算了進去**。

兩者在畫面上長得一模一樣。現在推估的那一種：欄位標題改成「推估發言時間」，
表格底下多一行講清楚它是怎麼算的。

> **判準由伺服器端給**（用了哪一種解析方式），前端不要自己從段落猜 ——
> 「每一段的結束時間剛好等於下一段的開始」在真的字幕檔上也可能成立，
> 猜出來的判準會在某些檔案上說謊。
>
> 檢查**先證明那個前提成立**（純文字的結束時間真的是下一段的開始）——
> 哪天解析器改成不補，上面那條的期望值就會變成在說謊，而它照樣全綠。

### 卡片空的時候不要替會議下結論

原本寫「這場會議沒有這一類的內容」，改成「**分析沒有在逐字稿裡找到**這一類的
內容」。抓到率量出來是 94~100%，不是 100% 保證 —— 前者是替會議下結論，
後者才是我們真正知道的事。

## [1.15.89] - 2026-09-20

### 送件前檢核：「我的作業」那一列按不了任何東西

那一列只有兩個出口：**下載**（`result_path`）與**開啟**（`view_url`）。
送件前檢核的產出不是一個檔案而是**案件底下的一份報告**，所以本來就沒有
`result_path` —— 而它也沒有 `view_url`，於是那一列顯示「完成」卻什麼都按不了，
使用者得自己想到要回工具裡翻案件。

案件頁本來就按 case_id 定址、自己會輪詢進度、也有權限檢查，所以指過去就好，
**不需要做 `?job=` 還原**。

**盤點過全部 29 支走背景作業的工具**：真的有「開啟」的只有 3 支
（逐句翻譯、文件翻譯、會議摘要），而且三支都接得回來 —— 先前修的那個
「按開啟進去一片空白」在別的工具身上沒有再發生。兩個出口都沒有的就這一支。

> **⚠ 我第一次盤點是錯的**：用 `"view_url" in 原始碼` 掃，而 **`preview_url`
> 含有 `view_url`** —— 七支只有預覽圖的工具被判成「有開啟」，其中三支還被我
> 當成「有開啟但接不回來」的 bug。子字串比對又一次（本專案在前綴 / 子字串上
> 已經吃過好幾次虧）。檢查改走 AST，並且**專門有一條驗它不會把 `preview_url`
> 看成 `view_url`**。

> **檢查的「先證明掃得到東西」當場抓到我自己**：第一版的 AST 比對只認
> `_jm.job_manager.submit`（`Attribute`），而多數工具是
> `from … import job_manager` 之後直接呼叫（`Name`）—— **29 支只掃到 3 支**。
> 沒有那條的話，這份檢查會靜靜地只檢查三支工具，而且全綠。

> **判準要落在「我的作業」真的拿得到的資料上**：`view_url` 是在 `submit()`
> 之後才設的，跟清單之間隔著一次資料庫寫入。所以除了靜態檢查，另有一條
> 真的送一件作業進去、等它落到資料庫、再確認清單那一側讀得到。

## [1.15.88] - 2026-09-20

### 我的工作區收純文字（.txt / .md）

會議摘要的逐字稿、清單處理的輸出這類純文字，原本存不進工作區。

**判準是內容不是副檔名。** 工作區其餘每一種型別都有明確的訊號（PDF / PNG 看
魔術位元組、Office 檔開 zip 驗內部結構），純文字兩者都沒有 —— 照副檔名收的話，
任何東西改名成 `.txt` 都進得來，而型別檢查本來就是為了擋這件事。
現在的判準是「整份 UTF-8 解得開、而且沒有 `\t\n\r` 以外的控制字元」，
檔名**只在 .txt 與 .md 之間二選一**。實測 PNG 改名成 `.txt` 仍然被認成 PNG。

**整份都要看，不可以只抽前面幾 KB** —— 前 8 KB 乾淨、後面接二進位的檔案很容易
做得出來，抽樣會讓它整個進來。

> 純文字沒有「第一頁」可以畫，縮圖一律回空白佔位圖 —— 但要**明講**。
> 掉進「當成 PDF」那條路的話，它會去找一個不存在的 `file.pdf`，
> 錯誤訊息變成「檔案不存在」，看起來像使用者的檔案掉了。

### ⚠ 同一份清單又寫了兩份：上傳區的 `accept`

`my_workspace.html` 的檔案選擇器兩處各寫死了
`application/pdf,image/png,.docx,…` —— 而伺服器端的清單從 v1.14.6 起就多了
試算表與簡報。症狀是**檔案選擇器把收得下的檔案濾掉**，使用者只會覺得
「這個檔不能傳」，沒有任何錯誤訊息可循。

改成由伺服器端算好送進樣板（同 `data-ws-exts` 的做法）。
頁面上那幾句「PDF / PNG」的說明也一併改成不會再漂的講法。

> **既有的檢查當場抓到我沒決定的事**：`test_all_workspace_office_types_are_covered`
> 原本是「除了 .pdf / .png 以外都必須有 Office 縮圖」，那個排除清單是寫死的。
> 加純文字時它紅了 —— 而那不是缺陷。判準改成驗**分割**：四條路徑互不重疊、
> 聯集剛好等於允許的型別。加新型別而沒決定縮圖怎麼辦時一樣會紅，
> 但「決定不做縮圖」也算決定。
...
處理的是使用者上傳的、不可信的檔案，所以每次轉檔都會在拋棄式設定檔裡種下
`DisableMacrosExecution`。實測才發現：每一支轉檔函式都帶著 `--safe-mode`，
而那個旗標會在啟動時**把使用者設定檔重設掉** —— 跑完之後那一項已經不在裡面了。

舊註解寫「`--safe-mode` 跟巨集無關」，那句話本身沒錯，
**錯在它跟我們種下去的設定有關**：safe-mode 會把整份洗掉。

> **判準是「跑完之後設定還在不在」，不是「我們有沒有寫出去」** ——
> 檔案確實有寫出來，所以只驗寫入的話這個洞永遠看不到。

嚴重度要講清楚：LibreOffice 出廠的巨集安全性本來就是「高」，headless 轉檔
也不會觸發自動執行，所以這是**一層本來就沒生效的縱深防禦**，不是可利用的漏洞。

`--safe-mode` 原本要防的兩件事已經有別的東西擋著：使用者自訂設定 ——
每次呼叫都是全新的拋棄式設定檔；當機復原提示 —— `--norestore`。
拿掉之後用三份真實檔案（docx / xlsx）逐項比對過**頁數、頁面尺寸、字數、
逐頁算圖的像素雜湊完全一致**。

### ⚠ 匯出的 PDF 一直是 Letter 不是 A4

台灣印的是 A4。Letter 短 18mm、寬 6mm，照 A4 邊界排的內容會跑掉。
影響的是所有「來源沒有頁面尺寸」的轉檔：Markdown 轉辦公文件、會議摘要的匯出、
純文字與 CSV。

* **CSS 的 `@page { size: A4 }` LibreOffice 的 HTML 匯入不理。**
* **`LANG` / `LC_PAPER` 也沒有用** —— 實測 `LANG=zh_TW.UTF-8` 只換掉長度單位
  （in → cm），紙張仍是 Letter；而且多數伺服器根本沒有產生 zh_TW 這個 locale。

真正有效的是在拋棄式設定檔裡種 `ooSetupSystemLocale`。**但不可以無條件套** ——
它同時是系統語系，會改變中日韓的字型 fallback：實測一份真實廠商表 docx，
套上去之後前導空白改用別的字型量寬度，整段標籤**左移約 10pt**
（墨點數與字形完全相同，純粹是空白的寬度）。表單那一家工具對座標位移極度敏感，
而它們的來源本來就寫著頁面尺寸、根本不需要這一項。

所以判準是**來源有沒有自己宣告頁面尺寸**：沒有的（HTML / 純文字 / CSV）才由我們
決定紙張；有的一律照它的（實測宣告 Letter 的 .odt 轉出來仍是 Letter，
真實 docx 的逐頁像素雜湊跟改動前完全相同）。

## [1.15.86] - 2026-09-20

### ⚠ 「估計」不是台灣用語

使用者當場糾正。台灣的說法是**預估**（預估閱讀時間）、**推估**（從別的
數字推算來的）或**約**（約 800 MB）。已進禁用詞表，判準是使用者看得到的
文字（註解與說明文字裡的「背景估計」是技術名詞，不在範圍內）。

補上之後當場抓到兩處：

- 字數統計的「閱讀時間估計」——**同一支工具的另一格早就寫著「預估閱讀」**，
  兩種寫法並存在同一個畫面上。
- README 講記憶體量測的那一句。

順手改掉作業清單的「估 800 MB」→「約 800 MB」——**那一格的英文譯文本來
就是 `about`**，只有中文那一半是例外。

> 這跟「同一個家族要一次掃完」是同一條：加禁用詞的時候要把**同義的舊寫法
> 一起掃**，不然畫面上會一半新一半舊，而且沒有任何東西會紅。

## [1.15.85] - 2026-09-20

### ⚠ 匯出 .odt / .docx 時，標題是白字看不見

「商務報告」主題的標題是「白字 ＋ 深色橫幅」，而 Office 引擎的 HTML 匯入
**保留文字顏色、但丟掉段落底色** —— 底色一掉就變成白字白底，整行消失。
PDF 沒有這個問題（底色畫得出來）。

**「Markdown 轉辦公文件」也一樣中招**，只是沒有人回報過。兩支工具現在共用
同一份覆寫：文件格式改成深色字 ＋ 底線，PDF 的橫幅維持原樣。

> 通則：**不要讓可讀性依賴底色**。底色是轉檔途中最容易掉的東西，
> 而掉了之後的症狀是「什麼都看不到」，不是「顏色怪怪的」。

### 會議摘要：貼上逐字稿時，文件標題改從會議背景取主題

貼上時的檔名是系統自己塞的「貼上的逐字稿.txt」，於是匯出的文件標題變成
「貼上的逐字稿 會議記錄」——那是內部說法，不該出現在要發出去的文件上。

現在的優先序是：**會議背景裡明寫的主題 → 檔名 → 一句通用的「會議記錄」**。
下載的檔名也跟著同一個來源。

**不從摘要猜主題**：摘要是一整段話，截前幾個字會斷在半句，而且那個標題
會隨著模型每次的講法變動。背景欄本來就是寫「會議主題：…」的地方。

### 其他

- 會議摘要的匯出區拿掉「轉送到 Markdown 轉辦公文件」按鈕（結果區已有 Word / ODF）
- LLM 設定頁的會議摘要說明改成：建議 gemma4:26b 或參數量更高的模型；
  顯示記憶體有限時 qwen3.8:27b 是實測可用的選擇

## [1.15.84] - 2026-09-19

### 會議摘要：點發言分布上的色塊，跳到**那個時間**講的那一段

原本整列共用一個段號（那個人第一次發言），所以不管點哪一格都跳到同一個
地方 —— 而使用者眼前看到的是某個時間點的色塊。現在每一格都帶自己的段號，
點空白處才退回這一列的第一段。滑鼠移上去看得到那一格是幾分幾秒、第幾段。

### 會議摘要：議題時間軸最長的那一列，百分比與時間不再折行

長條原本直接跟文字搶寬度，最長的那一列長條佔滿，文字就被擠成兩行。
改成長條畫在一條固定寬度的軌道裡 —— 文字永遠有位置，
而且每一列的比例是用同一條軌道算的，不會因為文字長短而不同。

## [1.15.83] - 2026-09-19

### 會議摘要：可以匯出 Word 與 ODF，而且版面主題可以選

除了 PDF，現在也匯得出 **Word（.docx）** 與 **ODF（.odt）**，
拿回去接著編輯、套公司範本都方便。

下載列多了一個**版面主題**的選單，六種（清爽 / GitHub 風 / 學術論文 /
書籍 / 商務報告 / 極簡黑白），預設是商務報告。

三種格式與六種主題都是**直接用「Markdown 轉辦公文件」那一套** ——
連渲染器與主題定義都共用。自己再寫一份排版的話，那一份一定會比既有的差，
而且那邊加主題時這裡不會跟著加。
...
上一版把畫面上的語者長條圖併進表格、換成「發言分布」，
**但匯出用的是伺服器畫的另一份** —— 於是網頁上是分布、PDF 裡還是舊的長條圖，
連未標示的講者都還顯示成 `unknown`。**同一份東西寫在兩個地方一定會漂**，
這次漂在「畫面」與「匯出」之間。

伺服器端也改成畫「誰在什麼時候講話」（一人一列，講話的地方就有一格），
匯出的表格排序與欄名也跟畫面一致，未標示的講者不再顯示代號。
公開 API 只給分析結果、沒有逐段資料時，仍然退回原本的長條圖。

### 會議摘要：引用的段號只放數字

卡片很窄，而一條項目常常有三四個引用 ——「第 10 段」三個字裡有兩個是
每一顆都一樣的裝飾。完整的說法留在滑鼠提示與螢幕閱讀器的標籤裡。

## [1.15.81] - 2026-09-19

### 會議摘要：議題時間軸的長度條貼近它的標題

標題與它底下的長度條是同一件事，原本中間空了一段，看起來像兩列。
間距來自兩處 —— 列的行高（一般內文用的 1.5，標題底下會多出三四個像素）
與長度條自己的上留白，**只調其中一個不夠**。

## [1.15.80] - 2026-09-19

### 會議摘要：匯出的 PDF 裡，圖不再超出版面

圖照原始像素放進去，心智圖有 980 像素寬 —— **右邊整片被切掉**。
`max-width: 100%` 沒有用（Office 引擎的 HTML 匯入不理 CSS 的這一條，
實測仍然照 980 放），要用 HTML 的 `width` 屬性。

**光限寬還不夠**：心智圖縮到頁寬之後仍然比一頁高，而引擎不會自己分頁 ——
它就放一張、超出的部分裁掉（實測圖底比頁底多 57 點，下面全部不見）。
太高的圖現在會自己切成好幾張，每一張都放得進一頁。

### 會議摘要：圖的標題不再出現兩次

圖自己畫著標題，外面的 Markdown 又加了一個同名的段落標題。

## [1.15.79] - 2026-09-19

### ⚠⚠ 會議摘要匯出的 PDF **從來沒有成功過**

畫面上的會議摘要排得好看，匯出的 PDF 卻完全不是那麼回事：拿到的是三頁圖、
**一個字的內文都沒有**，頁面尺寸還是 980×2640 這種數字 ——
那是圖的原始大小，不是紙張。

轉檔那支函式的第二個參數是**目標檔案**，而且**回傳 `None`**；
原本的程式碼把目錄傳進去、又把回傳值當成產出的檔案，
所以那段判斷**從來沒有成立過一次**，每次都退回「只有圖」的版本。
而回 `None` 不是例外，所以那個 `except` 沒有觸發 ——
**日誌裡一句話都沒有，下載永遠是 200**。

另外原本是把 Markdown 直接丟給 Office 引擎，而**它不認得 Markdown**。
現在先渲染成 HTML（跟「Markdown 轉辦公文件」共用同一支渲染器），
匯出的是一份有標題、摘要、決議、待辦、風險、議題與語者表格的文件。

**退化現在一定會留下紀錄**，說得出是哪一步失敗 ——
拿到「只有圖」的 PDF 跟拿到完整的那一份，在使用者眼裡都只是「檔案下載好了」。

### 會議摘要：文件標題不再帶副檔名

「第四季預算會議.txt 會議記錄」→「第四季預算會議 會議記錄」。

### 議題時間軸：章節改叫議題

「章節」是從影片、書籍借來的說法，會議裡沒有人這樣講。
卡片改叫「議題時間軸」，底下那張圖改叫「各議題佔多少時間」。

順帶修掉那個標題**從來沒有被翻譯過**的問題：它原本寫成
`tr(條件 ? 'A' : 'B')` —— **鍵是執行期算出來的，抽鍵的工具永遠看不到**。
現在標題搬到 HTML、逐個分支各自翻，也才放得下區塊圖示。

## [1.15.78] - 2026-09-19

### ⚠ 會議摘要：從「我的作業」按開啟，結果是空的

網址帶作業編號回來時，作業早就跑完了 —— 進度列寫著「完成」，
**但下面一片空白**，看起來像結果被清掉了。

原因是讀結果那一步需要上傳編號，而它是**上傳當下**才有的，
重新開一頁是空的，於是那段程式**安靜地什麼都不做**。
作業自己記著那個編號，現在會先把它拿回來再讀結果。

### 會議摘要：發言分布與統計合併成同一張表

原本是左邊一張圖、右邊一張表並排，要逐列對齊 —— 順序、列高、起點都得一致，
而列高受字型與瀏覽器影響，量出來對齊還是會差幾個像素，
而「差一點點」正是對不齊的樣子。

合併之後由結構保證對齊，一列就是一列。分布那一欄用百分比定位，
欄寬變了自己就跟著變。整列點得下去，跳到那個人第一次發言的地方。

## [1.15.77] - 2026-09-19

### 會議摘要：語者那張圖改成「誰在什麼時候講話」

原本是長條圖，畫的是「誰講最多」—— 而那件事**右邊那張表已經寫著了**，
再畫一次只是把同樣的數字換個形狀。

改成一人一列的時間軸：講話的地方就有一格。一眼看得出三件表格說不出的事 ——
**誰主導**（那一列的墨水最多，佔比仍然看得出來）、**節奏**（全場穿插，
還是集中在某一段）、**誰在哪一段沒出現**（空白處就是他沒講話的時候）。
...
百分比、時間、次數原本是串成一句一起靠右，前面那欄就被後面那欄的長度推著跑
（`9.0%` 與 `13.8%` 差一個字，整欄就歪了）。改成各自一欄靠右，
數字也改用等寬字形。

### 會議摘要：會議背景的輸入框套上樣式

原本只設了寬高與字級，邊框、圓角、內距都沒有，看起來是瀏覽器的預設方框。

### 會議摘要：未標示講者不再顯示 `unknown`

`unknown` 是分析時塞的代號不是名字。資料裡維持原樣（統計、引用、匯出都靠它
對應），畫面上顯示「未標示講者」。
...
原本寬度跟著內容跑，四格大小不一又靠左擠成一團，跟下面滿版的欄位對不齊。
改成等寬的格線撐滿整列，窄了自己折成兩排。

### 會議摘要：排法提示不再是兩層括號

排法的名字本身就帶括號（「一行一句（講者：內容）」），外面再包一層就變成
「（自動判斷為：一行一句（講者：內容））」。現在是「自動判斷：一行一句（講者：內容）」。

## [1.15.75] - 2026-09-19

### 會議摘要：語者表格加上格線

一列有五個數字，原本只有很淺的底線、沒有直線，眼睛接不起來。
補上橫線、直線與隔列底色，滑鼠移過去整列會反白。

### 會議摘要：表格的「佔比」改成「字數佔比」

旁邊那張圖用的是**發言時間**，表格的佔比一直是**字數** ——
同一個人的兩個數字並排（23.1% 與 21.8%）會讓人以為有一邊算錯了。
欄位名改成說得出是哪一種。
...
PDF / 圖表 PNG / ZIP 是**按下去才產生的**（把圖算成點陣圖、打包），
而那段 CPU 工作原本直接跑在網頁伺服器的事件迴圈上 ——
只要有人按下載，**其他人連首頁都打不開**，要等它算完。

已丟到背景執行緒。既有的「端點不可以鎖住事件迴圈」靜態檢查看不到這種形狀
（重活藏在被呼叫的函式裡），所以這支另外列進那份測試的清單釘住。

### 會議摘要：下載按鈕會說「產生中…」

原本是純連結，瀏覽器在收到檔案之前畫面完全不變 ——
按了像沒反應，使用者會再按好幾次。現在按下去立刻變「產生中…」並轉圈、
擋住重複點擊，拿到檔案才觸發儲存。

順帶換成自己抓回來，伺服器出錯時看得到訊息 ——
原本錯誤頁會被當成檔案存下來（擷取文字那支踩過同一個坑）。

### 會議摘要：章節時間軸的線斷成一截一截

軸線是每一列各畫一段，而它只撐到「內容框」—— 列有上下內距，
所以每兩列之間都空一段。溢出量現在跟內距綁同一個變數，改內距不會又斷掉。

## [1.15.73] - 2026-09-19

### 會議摘要：上傳卡片的順序改成符合實際流程

「會議背景」原本在「用這段文字」按鈕**下面** —— 等於按完才看到它，
順序是反的。改成「貼上逐字稿 → 會議背景 → 按鈕」，按鈕留在最後。

按鈕名稱也從「用這段文字」改成「解析逐字稿」：它做的事是把內容解析成
段落與講者（按完會顯示段落數、講者數與前幾段），名字要說得出這件事。

## [1.15.72] - 2026-09-19

### 會議摘要：「會議背景」看起來像獨立區塊，改成卡片裡的一節

那一區原本就在「1. 上傳逐字稿」卡片裡面，但它自己有邊框和底色，
看起來像**卡片裡又一張卡片**。拿掉那層框，改用一條分隔線收邊 ——
跟「或者直接貼上逐字稿」同樣的視覺重量，一眼看得出是同一張卡片的一節。

## [1.15.71] - 2026-09-19

### 會議摘要：可以填「會議背景」

上傳區底下多了一個選填欄位（預設收起來），拿來寫**逐字稿以外的資訊** ——
會議主題、時間地點、與會者與職稱、專有名詞說明，或是你自己想交代的話。

填了會讓分析更準：知道誰是主管、誰是承辦，「請某某去辦」才判得出負責人；
知道系統代號與縮寫，整理出來的句子才不會走樣。

**背景資料不會變成項目的來源。** 背景裡寫的事情沒有在會議上發生過，
如果它能生出決議，這支工具「每一條都指得回逐字稿」的保證就破了，
而且讀的人完全看不出來。三道防線：

1. 提示裡明講，而且**我們的規則夾在使用者文字的前後兩側** ——
   填的內容有可能寫得像指令，最後說話的必須是我們。
2. **引用驗證**：項目要指得回逐字稿的段落，而背景不在被比對的範圍裡。
3. **「跟背景很像，而且明顯比跟逐字稿更像」就丟掉**。

第 3 道是寫測試時才補上的 —— 原本以為第 2 道就夠，實測發現不夠：
使用者填的背景（議程、主題）談的本來就是會議上談的事，從議程抄一條決議出來，
它跟逐字稿的重疊率照樣過得了門檻。

> 這一道**擋不住**「背景與逐字稿講法差很多、模型自己把兩邊揉在一起」的句子。
> 背景資料仍然是一個要小心的輸入。

公開 API 也收了（`context` 參數，上限 4000 字）。

## [1.15.70] - 2026-09-19

### 會議摘要：圖與版面一輪

- **章節改成時間軸**：左邊一條軸線、每一章一個點，右邊除了標題再畫一條長度條。
  原本是「時間 ｜ 標題」兩欄的表，標題只有十幾個字，右邊空掉一大半；
  現在同樣的資料，但「哪一段談最久」一眼看得出來。
- **滑過長條，對應的圖例亮起、其餘變淡**，看得出是在對應哪一個。
- **語者佔比的圖與表左右並排**，視窗窄了自動往下排。
- **上傳後的統計做成有底色的區塊**（段落 / 講者 / 字數 / 總長）。
- 圖例的字放大了一級。

### 修正：圖的字忽大忽小

同一天收到兩個方向的回報 ——「字又過大」與「寬度不夠時字又過小」。
**兩個的根因是同一個**：圖畫出來的寬度跟它在畫面上實際佔的寬度對不起來，
於是整張圖連字一起被 CSS 縮放。

字太大是因為心智圖那一區**在量寬度的當下還是隱藏的**（量到 0，退到預設值，
再被撐大 1.7 倍）；字太小是反過來，畫得比容器寬又被壓下去。

現在圖照實際像素畫，而且**畫完會量一次**，跟實際寬度差超過 2% 就照實際寬度
重畫一次。量錯的來源很多（容器還在攤開、字型還沒載完、捲軸讓寬度少幾個像素），
與其一個一個堵，不如畫完照鏡子。

### 修正：心智圖右邊那些項目，滑鼠移上去沒有反應

左邊的章節有亮起來、右邊的項目沒有。那些框的底色本來就是白的，
而原本的效果是調整透明度 —— 在白色頁面上調透明度**看不出任何差別**。
改成調亮度，白底跟彩色長條都看得出來。

### 修正：進度列多出一顆「下載 .json」

會議摘要的結果區自己有一整排下載按鈕（PDF / Markdown / 圖表 PNG / ZIP /
JSON / 轉送），而共用進度列又冒出一顆「下載 .json」，看起來像是別的東西。
共用進度列現在可以由工具決定要不要顯示那顆按鈕。

另外，進度列的按鈕跟下面的卡片之間補了留白（全站的背景作業都一起）。

## [1.15.69] - 2026-09-19

### 修正：按了「停止」，畫面看不出來停了

按下停止之後，時間確實不走了，但**進度條停在原處、狀態文字還寫著
「擷取重點 4/5」、「可以關掉這一頁」也還掛著** —— 跟「卡住了」長得一模一樣。

原因是停止的動作只做了「停止輪詢 ＋ 通知伺服器」，而**把畫面畫成「已停止」
的程式碼正好在輪詢那條路上**（讀到伺服器回報已取消才會執行）。
一停輪詢，那段就永遠不會跑到。

現在按下去會立刻：狀態寫「已停止」、進度條轉灰、收起「可以關掉這一頁」。
**已過時間留著** —— 那是「跑了多久才停」，是有用的資訊。
這是全站共用的進度列，所有背景作業的工具都一起受惠。

### 修正：會議摘要的貼上框，提示文字印出 `&#10;`

「直接貼上逐字稿」那個輸入框的提示文字，畫面上直接印出 `&#10;` 而不是換行。

原因是那段提示包在翻譯函式裡，而**翻譯出來的字會自動跳脫** ——
`&` 被換成 `&amp;`，瀏覽器看到的就是那六個字。三種語言的語系檔都跟著
抄了同一個寫法，所以三種語言都一樣。
...
三張圖（討論結構、章節佔比、語者佔比）以前是伺服器畫好的圖片，
所以**裡面的東西點不動** —— 而這支工具的賣點正是「每一條都指得回原文」。
現在改成在瀏覽器裡畫：

- **點圖上的任何一格，逐字稿就跳到那一段並標起來**（跟卡片上的引用同一支邏輯）
- 跟著容器寬度畫，視窗縮放會重畫，寬螢幕上不會右邊留一大片空白
- 匯出的 PNG / PDF / Markdown 內嵌仍然是伺服器畫的那一份

### 會議摘要：討論結構右半邊不再是一片空白

決議與待辦幾乎都集中在會議後段，所以十個章節常常只有三個掛得上東西，
另外七列的右半邊是空的，看起來像圖畫壞了。那些章節**確實沒有產出**，
所以不是隱藏也不是補一個節點，而是換一種畫法：畫成一條橫跨整個寬度的扁條，
右邊註明「無決議／待辦」。一眼就分得出「談過但沒有結論」與「談出東西來了」。

### 修正：明明有時間戳記，章節佔比卻說沒有

純文字逐字稿的時間是行首的 `16:19` 這種標記，而**不是每一行都有**
（開頭的抬頭、「（短暫沉默）」那種行、以及最後一段都拿不到）。
原本計算章節時間時寫死「第一段的開始」與「最後一段的結束」，
只要章節的頭尾剛好落在那種行上，整章就沒有時間 ——
而只要有一章沒有時間，**整張圖就退回用段數**，畫面上寫著
「這份逐字稿沒有時間戳記」。

現在改成在**有時間的那些段**裡取最早與最晚，那是真的資料不是猜的。

### 會議摘要：逐字稿整份攤開、卡片標題更明顯

逐字稿原本關在自己的捲軸裡（點引用跳過去變成在一個小視窗裡捲動，
而且用頁面捲軸看不到後面的內容），現在整份攤開。
決議／待辦／風險／未決問題四張卡片的標題加大、加底色與左邊色條，
數量做成徽章 —— 四張並排時一眼分得出是哪一類。

### Windows 批次檔的行尾：補上檢查

`setup-python.cmd`（Windows 安裝時建立 Python 環境用的批次檔）**必須是
CRLF 行尾** —— LF 行尾的 `.cmd` 在 cmd.exe 會逐 token 噴錯，於是機器上
沒有裝 git、走壓縮檔下載的人整個安裝會失敗。v1.12.8 / v1.12.10 修過這件事，
修法有三道（安裝前先正規化、檔案改成 CRLF、`.gitattributes` 宣告規則），
**但一道檢查都沒有**。

這一版加了 `tests/test_script_line_endings.py`：`.cmd` / `.bat` 每一行都要
CRLF、`.sh` 一個 CR 都不可以有，而且 `.gitattributes` 的那三條規則要還在
（用 git clone 的人靠的是它）。

用 git clone 的人本來就看不到這一類問題 —— checkout 時會自動轉回 CRLF，
所以開發機上怎麼測都是對的。

### 其他

- 部署用的壓縮檔改成**允許清單**（只裝該裝的），而且建完會打開來驗過才交出去
- 內部工具與自動檢查的整理
...
上傳的檔名如果在副檔名前面多一個空格（`會議紀錄逐字稿 .pdf` —— 從網頁複製
檔名時很常見），TXT / Markdown / Word / ODT 四個下載按鈕**全部下載不到東西**，
「存至工作區」也一起壞掉。瀏覽器會把錯誤訊息當成檔案存起來，
存檔對話框顯示的是 `txt.json`。

原因是**寫檔案時用原始檔名、讀回來時卻先去掉頭尾空白**，兩邊組出來的路徑
不一樣。訊息寫的是「已過期」，看起來像檔案被清掉了，完全不像檔名對不上。

下載的檔名也補上了純英數的備用名稱：只送 UTF-8 檔名時，有些瀏覽器會改用
網址的最後一段當檔名。

### 擷取文字：預覽區可以一鍵複製

複製的是預覽的內容（前 5000 字），按鈕會告訴你複製了多少、整份要用「下載 TXT」。

### 會議摘要：這一版改了很多

**逐字稿的格式自動判斷，而且可以自己指定。** 支援八種常見排法：講者標頭
自己一行（下一行才是內容）、一行一句、時間寫在前面或括號裡、清單符號開頭、
半形冒號等等。畫面上會顯示判斷出來的格式，判斷錯的時候可以自己換。

**可以直接貼上逐字稿**，不必先存成檔案。

**超長的發言會自動切段。** 委員會逐字稿常常一個人連講好幾分鐘 ＝ 一段兩三萬字，
以前所有引用都會指向「第 1 段」，點過去看到一大片文字，等於沒有出處。
現在一段最多 400 字（實測：一份 26,907 字的紀錄從 4 段變成 78 段）。

**很長的會議不會再出錯。** 以前送給模型的內容會跟著發言長度成長，超過模型
能讀的範圍時是**安靜地被截斷**（看起來成功，其實只讀到後面一部分）。
現在送出的內容有上限，複審也改成分批進行，一批失敗只影響那一批。

**心智圖、語者佔比、章節佔比改由伺服器繪製**，畫面、Markdown、PDF 用的是
同一份圖。心智圖是**組出來的不是生成的** —— 每個節點都來自通過出處驗證的
項目，所以圖上每一格都點得回逐字稿。

**沒有時間戳記也看得到「誰講了多少」**（發言次數與字數），以前整個區塊會消失。
章節也一樣：以前沒有時間就整個不見。

**新增匯出**：PDF（整份會議記錄）、圖表 PNG、Markdown ＋ 圖的 ZIP。
下載的 Markdown 直接內嵌圖片，轉送到「Markdown 轉辦公文件」時圖還在。

**修正**：「轉送到 Markdown 轉辦公文件」按了沒反應。每一段與每張卡片加上圖示，
匯出按鈕與下面的內容留開。

### 首頁顯示工具數

「整合式 PDF / Office 文件處理平台 **49** 支工具」—— 這個數字是算出來的，
加工具就會自己跟著變。

## [1.15.66] - 2026-09-18

### 修正：開啟「API token 強制檢查」之後網頁介面會卡住（issue #52）

強制檢查一開，網頁自己打的 `/api/` 請求全部被拒絕 401 —— 進度輪詢、取消
作業、通知、收件匣、工作區清單。**頁面本身打得開，作業也真的在背景跑完了**，
所以看起來像是「進度不動」「按了沒反應」，完全不像設定造成的。

原因是判斷方式只認一份**寫死的路徑清單**（管理區與另外兩支預覽），
清單以外的都被當成外部 API 呼叫。現在改成看**這個請求有沒有帶登入後的
session** —— 有就走原本的權限檢查，沒有（腳本、`curl`）才擋。
不再維護那份清單，之後新增的端點不會再漏掉。

同時把設定頁的說明改成事實：**認證關閉時這個選項沒有作用** ——
那時候整個站本來就不需要身分，無法分辨「網頁」與「腳本」，兩者都會放行。
要真的限制外部呼叫，請先啟用認證。原本的說明只寫「一律拒絕 401」，
在認證關閉時是不成立的。
...
把會議逐字稿整理成**摘要、決議、待辦、風險、未決問題與章節**。

**每一條都指得回原文的第幾段、誰講的** —— 點下去就跳到逐字稿那一行。
這不是裝飾：會議記錄會被拿去當依據，**一條沒有出處的決議比沒有那條更糟**。
抽出來的內容如果在它宣稱的段落裡找不到，就不會留下來，
結果頁會告訴你丟掉了幾條。

收得進來的逐字稿：字幕檔（`.vtt` / `.srt`）、逐字稿 JSON、
純文字（`.txt` / `.md`）、Word 與 ODF（`.docx` / `.odt`）。
純文字一行一句話，前面可以寫「講者：」，有時間戳記也讀得到。

**上傳之後、開始分析之前會先給你看解析結果**（段落數、講者、前幾段長什麼樣）
—— 講者判錯要在花掉那幾分鐘之前就看得出來。

有時間戳記時另外算**語者發言佔比**，是由時間直接算的（重疊的插話只算一次），
不是估的。**沒有時間戳記時這張圖與章節時間軸就不會出現**，畫面上會說明為什麼
—— 猜出來的數字不能用。

其他：分析中可以停止；關掉分頁也跑得完，從「我的作業」按「開啟」接得回來；
下載的 Markdown 可以直接轉送到「Markdown 轉辦公文件」排版。
需要先在管理區啟用 LLM。
...
### 新增 `/readyz`：服務「還活著」跟「東西都在」是兩件事

工具載入失敗（多半是缺相依套件）原本只在記錄檔留一行，然後那支工具就安靜地
不見了 —— 側欄少一項、網址進去 404，而 `/healthz` 照樣回 `{"ok":true}`。
管理員沒有任何地方看得到。

新增 `GET /readyz`（跟 `/healthz` 一樣不需登入）：

- 有工具沒載入 → 回 **200 ＋ `degraded: true`**，其餘的工具照常可用
- 資料目錄寫不進去、或資料庫開不起來 → 回 **503**（那才是真的不能工作）
- **不會吐出模組名稱、例外訊息或檔案路徑** —— 這支是公開的

**哪幾支工具沒載入、原因是什麼**，顯示在管理區的**系統狀態**頁最上面
（需要管理員），沒有失敗時完全不占畫面。補裝套件之後要重新啟動服務才會掛上來。

`/healthz` 維持原樣，**刻意不看**工具載入狀況 —— 服務管理員拿它決定要不要重啟，
因為少一支工具而一直重啟比問題本身更糟。同理少幾支工具時 `/readyz` 不回 503：
本產品是單一 web 行程，把唯一的實例判成不健康，使用者看到的會是整站錯誤頁。

監控設定寫在 `OPS.md`。這一條來自外部原始碼稽核的建議。

## [1.15.63] - 2026-09-18

### Markdown 轉辦公文件：改成背景作業，轉換中可以停止

原本是同步請求 —— 畫面上只能盯著「轉換中…」，沒有任何辦法停下來，
關掉分頁還會白做一次。

現在有進度列與「停止轉換」，而且關掉這一頁也跑得完，之後從「我的作業」
按「開啟」就接得回來。停止會把轉檔程式整棵停掉（v1.15.62 的機制），
不是只把畫面藏起來。

完成之後「我的作業」也有下載鈕了，檔名用你填的標題。

## [1.15.62] - 2026-09-18

### 修正 v1.15.61 造成的退步：程式碼區塊超出頁面

上一版為了讓程式碼與外框之間留白，把程式碼區塊包進了單格表格。結果是
**長的指令不再自動折行，整塊被撐到超出頁面右緣、內容被切掉**。

內容被切掉比字貼著框嚴重得多，所以這個做法已經退掉。實測過三種取得留白的
方法（`padding`、外層容器、單格表格），轉檔引擎都給不了想要的效果。
現在改成把整塊底色帶往右縮排、不畫外框 —— 沒有外框就沒有「貼著框」，
而縮排讓程式碼與正文明顯分開。

表格的框線不受影響，仍然是上一版加上的可見框線。

### 按下取消時，真的把工作停下來

「我的作業」與工具頁上的停止按鈕原本只把狀態改成「已停止」——
轉檔那幾分鐘整個卡在等待外部程式，程式裡的取消檢查點永遠不會被執行到。
畫面顯示已停止，而**伺服器照樣把那一次算完**，繼續佔用 CPU 與記憶體。

現在取消會把該作業啟動的外部程式整棵停掉。這對所有會呼叫外部程式的工具
都有效（文字辨識、各種轉檔）。

停掉之前會比對行程的身分 —— 長時間運行的服務行程編號會被重複使用，
不比對的話有機會停到不相干的行程。

## [1.15.61] - 2026-09-18

### Markdown 轉辦公文件：表格有框線了，程式碼不再貼著框

轉出來的表格沒有框線、欄寬也不好看；程式碼區塊的文字緊貼著外框。

原因是轉檔引擎對 CSS 的支援很窄：`border: 1px solid …` 這種寫法它**完全不畫**，
而 `padding` 套在程式碼區塊上會被當成縮排（框和文字一起往右移，中間還是沒有空隙）。
改用 HTML 的表現屬性之後就穩定了 —— 表格框線實測 0.75pt（看得見），
程式碼區塊的文字與外框之間有約 6pt 的空白。

同一份文件也順帶少了一頁（27 → 26 頁），而且原本重疊在一起的兩組框線
（一組來自屬性、一組來自 CSS 的極細線）不再互相干擾。

## [1.15.60] - 2026-09-18

### Markdown 轉辦公文件：分頁不再出現整頁空白

轉出來的 PDF 會出現只有一兩行字的頁面，表格被切成一列一頁。原因是轉檔時
走的是「網頁檢視」排版而不是文書排版 —— 同一份文件實測 36 頁、其中 3 頁
只有 25～53 個字，改用文書排版後是 25 頁、沒有任何空白頁，每頁字數中位數
從 687 提高到 1029。表格的欄寬也正常了。

其他兩種輸出（.docx / .odt）早就用文書排版，只有 PDF 這條路漏掉。

### Markdown 轉辦公文件：可以只轉需要的格式

原本每次都轉 PDF ＋ DOCX ＋ ODT 三種。三種各要跑一次轉檔引擎，而引擎是
序列化的、每次上限 120 秒 —— 主機忙的時候一份幾十 KB 的 Markdown 就可能
超過兩分鐘然後失敗，而畫面上寫著「可能要 10-30 秒」。

新增「輸出格式」勾選，預設只轉 PDF。畫面也說明了頁面預覽是從 PDF 產生的，
沒有勾 PDF 就不會有預覽。對外 API 沒有指定格式時維持三種全轉，既有呼叫
不受影響。

### Markdown 轉辦公文件：程式碼區塊有語法上色

程式碼區塊會依標示的語言上色，行內程式碼的顏色也保留。
「極簡黑白」主題刻意不上色 —— 那個主題的用途就是完全中性的視覺。

### 轉檔逾時之後，轉檔引擎沒有真的被停掉

轉檔逾時時只停掉了外層的啟動程式，真正在解析檔案的行程會留下來繼續佔用
CPU，而且不會自己結束。因為轉檔行程被刻意降低優先權（讓網頁保持回應），
留下來的行程會讓後續每一次轉檔都更慢、更容易逾時。現在會停掉整棵行程樹。

影響全部七條轉檔路徑（辦公文件轉 PDF、轉圖片、Markdown 轉辦公文件等）。

### 轉檔逾時的訊息不再指向錯誤的方向

Markdown 轉檔逾時時，訊息寫的是「檔案可能已毀損，請對方提供 PDF 版」——
但那條路的中繼檔案是本工具自己產生的，跟使用者上傳的內容無關。
改成說明逾時通常是主機忙碌或文件較大造成的。
...
> 之後，固定成本降到 **約 0.5 秒**。檢查盯著那支核心的 import 清單。
...
收成 `tools/browser_probe.py` 一份，八支全部改用它，並加檢查擋第九份。
...
> 檢查的判準是「**真的送一份 Office 檔進去、而且讓 soffice 找不到**，看回傳碼」，
> 不是掃原始碼有沒有寫 503。變異驗證過：拿掉修正 → 回 500 → 檢查紅。
...
> **檢查走宣告的語言清單**（`tests/test_installer_product_name.py`）：
...
才出現。新檢查 `tests/test_ternary_branches_go_through_tr.py` 逐檔掃，
四個方向變異驗證過。
...
（台灣要寫「信箱」）—— 而那正是我們自己禁用詞表上的詞，**只是用詞檢查
沒有掃到那個資料夾**。示範資料正是要拿去拍截圖公開的。

改成從出貨的那份取（單一來源），用詞檢查的範圍也一併補上。
...
* 兩支安裝程式檢查的「哪些語言的譯文可以有漢字」收成一份共用的判準。
...
檢查只掃**正規式**那幾行，`tests/` 與 `tools/` 一起掃。
...
`patterns.default_doc_lang`，檢查走 AST 驗「有沒有真的委派過去」。
...
* **升級說明補一句「`-C` 是大寫」**（2026-09-14 客戶回報）：打成小寫的
  ...
  **那個訊息看起來像「這個安裝不是用 git 裝的」，其實只是打錯一個字母**，
  ...
  並加檢查擋住「可複製的指令區塊裡出現小寫的 `git -c <路徑>`」。
...
> **生成器與檢查的語言清單一律讀 `ui_locale.SUPPORTED`**。原本有八個地方
> 各寫死一次 `en` —— 那些檢查在加了第三種語言之後會**安靜地只驗英文**
> （「掃 0 個檔」跟「掃過都乾淨」在 pytest 輸出裡長得一模一樣）。

**加日文才現形的三件事**（英文那一輪都不會發生）：

* **用詞檢查把日文整份誤判。** 日文的「保存」「字体」是正確的日文，卻正好
...
> **共同點是「文字來自伺服器送來的資料」** —— 樣板掃字面 `tr('…')` 的檢查
> 一律看不到。新的檢查 `test_option_labels_go_through_tr` 直接驗
...
> **檢查兩層**：核心層比對「用轉向後的座標裁出來」與「直接在轉向後的圖上
...
> **兩支檢查**：`tests/test_no_svg_dot_hidden.py`（靜態，全站掃「用 `.hidden`
...
> （它的用途正是連回中文版）。檢查 `tests/test_troubleshooting_page_is_reachable.py`
...
檢查 `tests/test_update_fetch_survives_moved_tags.py`（走 AST，兩條變異各驗一次）。
...
> **既有檢查一條都抓不到**：元素都在、沒有 JS 例外、沒有殘留中文，截圖也看不
> 出來（手柄看起來就在紙角上）。新增
...
判準要落在**唯一來源**上：規則寫進同步腳本，檢查驗的是腳本而不是產出
...
> **檢查第一版被自己的解析騙了**：`class="tile-color-{{ t.color }}"` 這種
...
判定失敗。顯式 `SetErrorLevel 0` 修正，檢查變異驗過。
...
也沒有殘留中文**，既有檢查一條都抓不到。全站實算後同樣的情形還有 15 排。
...
檢查 `tests/test_button_icons_are_consistent.py` 兩條。

> **「同一排」一定要用位置判斷**：我第一版用 `<button.*?</button>` 連續比對，
> 非貪婪會跨過中間的 `</div>`，把根本不相鄰的按鈕算成一組（誤報 39 組）。
>
> **第一次變異沒紅要去追為什麼**：我原本規定「兩個中文字以上才算動作鈕」，
> 而「轉 180°」只有一個字 —— **這條檢查原本抓不到使用者回報的那一顆**。

### ⚠ 英文文件停在 34 個版本以前，而三條檢查全是綠的（使用者回報）

`CHANGELOG_en.md` 最新條目停在 1.15.4（2026-09-05）。既有檢查只驗
「檔案在不在、有沒有殘留中文、語言連結指不指得回去」—— 一份過期 34 版的文件
**那三條全部都通過**。又是同一句：記了規則但沒有檢查，等於沒記。

* 補上 1.15.7 ~ 1.15.38 共 32 條英文摘要。
* 三條新檢查（判準都能自己算，不寫死期望值）：
...
> 所以檢查補上**真的用 `makensis` 編一次**：這台幾秒就編完，判準是
...
另外補一支靜態檢查 `tests/test_template_script_deps.py`：行內腳本 `new` 了哪個
...
語系檔、README、介紹站、測試計畫全部換成「線上」，並加進用詞檢查。
...
檢查 `tests/test_settings_atomic_write.py` 逐檔掃 **AST 的 Call 節點**
...
> 是同一個病。檢查 `tests/test_office_convert_output_first.py` 用假 soffice
...
> **對照層的檢查管不到這一類。** 既有兩支測試驗的是「端點有沒有寫進文件」
...
- 測試計畫 §4.7 的「共 N 支」寫 267、實算 **265** —— 同一種病第 N 次
  （文件裡的數字沒有檢查）。改成**從路由表實算比對**，並把新工具的五支端點
  從章節開頭挪進工具清單裡它該在的位置。
...
測試計畫與 API 文件的涵蓋檢查 —— **檢查本身原本是假的**。

### ⚠ 涵蓋檢查靠「路徑尾段」比對，等於沒在檢查
...
涵蓋檢查原本把整個 `/admin` 前綴跳過（只守頁面本身）。實算之後：103 支會改
...
檢查補兩條：全路由對照（**兩邊都把路徑參數正規化再比**，不然 `{_filename}`
...
檢查 `tests/test_ops_iis_prereq_order.py`：標題與內文都要照正確順序，而且
...
檢查 `tests/test_preview_is_not_the_result.py`：**每一支只出部分預覽的工具**
...
到每次都會踩到 —— **偶爾紅的檢查跟壞掉的檢查一樣沒人信**，要嘛每次紅、
要嘛每次綠。
...
- 檢查 `tests/test_update_backup.py`：不可重建的資料**永遠不可以**進跳過
  清單，順序也釘死。
...
- 檢查 `tests/test_cli_data_dir_ownership.py`：用 **AST 找真正的呼叫**
  （只寫在註解裡不算數）。
...
**這兩件為什麼檢查沒抓到**：那條檢查只認 `python …` 開頭的指令，而計畫裡
...
已補進宣告與三處相依煙霧測試，並新增檢查 `tests/test_declared_dependencies.py`
...
涵蓋檢查第一版用字串比對「這個函式裡有沒有提到 `zip_guard`」，
而我在每個接上防護的地方都寫了一行註解：

    # zip 炸彈：判斷集中在 `zip_guard`，全站一份

變異驗證時只刪掉 import 與呼叫、**留下註解 —— 檢查照樣全綠**。也就是說那條
檢查從寫出來就是假的。改用 **AST 找真正的呼叫節點**（而且要分辨
...
> **第一次沒紅要去追為什麼。** 我第一次跑變異時檢查是綠的，當下若解釋成
...
有意思的是那支檢查**本來就寫著容許 503**（`status_code < 500 or == 503`）——
...
補上 gate，並加一條**檢查的檢查**：用 AST 找出「函式體裡真的呼叫了會起
soffice 的入口」卻沒掛 gate 的測試。判準是**它有沒有碰那幾支入口**，
不是猜名字，所以下一支新測試漏掛也會被抓到。

> 這條檢查的第一版用正規式，誤報了兩個：一個是**掃到自己**（判定用的字串
...
物件」，那些逐路由參數化的檢查會**縮成三條然後全部綠燈** —— 這個專案已經被
「空轉還全綠」騙過好幾次。
...
**管理員自己改過的一個字都不動**。檢查兩條（其中一條用「舊安裝」的資料庫實跑），
變異驗證過。
...
檢查 `tests/test_windows_git_guidance.py` 兩條（`--clear` 要在、
...
- **`test_no_sample_names_in_public.py` 在公開樹整份 skip**：路徑存成相對於
  ...
  檢查過，而「全部 skip」在輸出裡跟「全部通過」很像。另加一條檢查驗它自己
  沒有整份 skip。
...
- **「工具 id 有出現在計畫裡」不等於「有驗收」**：原本兩段「補列」把十幾支工具
  擠成一行一條，檢查照樣綠燈。19 支工具補上自己的驗收區塊、7 支加厚，
  並新增檢查要求每支工具至少三條驗收項。
- 新增檢查：非管理區的頁面也要有驗收、程式端產生的顯示字串（`tr(變數)`）
  都要有英文、屬性不可以原樣印出 `tr(…)`、顯示文字不可以寫 markdown。
- 英文版介紹站改用**英文介面的截圖**（15 張），並加檢查擋回頭用中文截圖。
...
檢查兩條：同一組內不可以有兩支工具用同一個圖示（變異驗證：把騎縫章改回
...
檢查 `tests/test_preview_page_range.py`：**判準是「不是 5xx」不是「一定要 404」**
...
- **`test_admin_users_table` 的表頭掃描沒認 `{{ tr('…') }}` 包裝** —— 加 i18n 的
  ...
  **檢查在無關的改動上紅，跟檢查漏掉真問題一樣糟** —— 下一次它紅的時候沒有人會信。
...
另外三條檢查：JS 的 `tr()` 鍵要有譯文、鍵裡不可以有樣板語法、
...
（`免責聲明` 拿到的是上一條的譯文）。**既有檢查只驗「有沒有殘留中文」——
錯位之後一個中文字都沒有，所以全綠。**
...
測試就會變，會讓「一覽表過期」的檢查在每次寫測試時都紅一次，純噪音。
...
新增檢查：徽章不可以低於 `tests/` 裡 `def test_` 的個數。刻意做成單邊判準
...
> （測試用 f-string 組路徑時它看不到）。檢查一律用**字面比對**
...
檢查 `tests/test_docs_english_pages.py`：英文頁不可以有中文殘留、語系檔不可以
...
**領域資料檢查改寫**：原本擋「語系檔裡不可以出現某些關鍵字」—— 那是錯的判準。
...
`is-ok` / `is-bad`），值寫在 CSS 裡。新檢查 `tests/test_generated_css_valid.py`
...
> `tr()`，並加自動檢查釘住「不可以叫 t」「不可以有樣板拿 tr 當迴圈變數」。
...
新增檢查 `test_llm_card_count_matches_the_tool_count`：介紹站的 LLM 卡片數必須
...
檢查 `tests/test_no_native_dialogs.py`：樣板裡不可以走原生 `prompt` / `confirm`
（`showXxx` 不在時的退路寫法仍允許）。
...
檢查 `temp/perm-cdp/cdp_table_align.py`：逐頁比對每一欄「標題的對齊方向」與
...
只有真的用眼睛看的人才發現不對。所以自動檢查不能只靠「有沒有例外」：
...
檢查 `tests/test_static_image_budget.py`：自家的介面圖片單檔不可超過 200 KB。
...
看不到缺了什麼。新增檢查 `tests/test_dependency_declaration_sop.py`：從
...
看得出來）→ 新檢查 `tests/test_db_query_plans.py` 直接驗資料庫的查詢計畫，
熱路徑不可以出現整表掃描。
...
檢查 `tests/test_no_blocking_endpoints.py` 從「只准變少」改成**必須是 0**。
...
新增檢查 `tests/test_test_plan_coverage.py`：工具、API 端點、管理頁、計畫裡
...
自動檢查 `tests/test_no_blocking_endpoints.py` 釘住這兩支，並用一個計數擋住
...
自動檢查 `tests/test_heic_support.py`（17 項）。核心那條是**從副檔名白名單自動
...
自動檢查 `tests/test_admin_exception_leak.py`：**自動列舉**管理區與 web 路由，
...
自動檢查 `tests/test_windows_service_restart.py`（6 項，假 SCM 模擬
...
自動檢查 4 項（直式 / 橫式 × 有無固定句，另加帶 footer 一項），判準是**框內
...
自動檢查 `tests/test_audit_timezone.py`（12 項），**固定把行程時區設成 UTC**
—— 不固定的話，在剛好設成 Asia/Taipei 的機器上，修正前的程式也會過。
另有靜態檢查（`ast`，不掃註解）擋管理區再出現不帶時區的時間解析，以及一條
...
自動檢查 `tests/test_license_declaration.py`（7 項）—— 授權宣告散在七個地方，
少改一處就是對外的法律宣告不一致。
...
日期 / 申請人 / 份數與邊框不受影響。自動檢查比對的是**實際章面尺寸與墨水**，
...
自動檢查 `tests/test_ou_key_canon.py`（12 項）。變異驗證做了兩輪：拿掉寫入端 →
...
自動檢查 `tests/test_directory_schema_matrix.py` 新增第四種結構「AD 相容但少
...
自動檢查 `tests/test_cjk_font_notice.py`（12 項）**自動列舉**會把中文畫進
...
自動檢查 `tests/test_windows_git_guidance.py`：擋「退回只依賴 winget」、
...
- **選擇視窗的樣式被 CSP 擋掉**。視窗樣式是 JS 動態注入的 inline
  ...
  樣式搬進 platform.css，JS 只建 DOM；新增自動檢查禁止前端動態注入
  `<style>`。
...
**用詞自動檢查有兩層都漏掉了**，所以這些字才留到現在：掃描清單只含
...
變異驗證（把修正逐一還原，確認自動檢查會變紅）—— 這一步抓到兩個我自己寫的
空殼測試。
...
- 新增用語自動檢查：工具名稱與描述不可以再出現舊說法；**只輸出單一格式的
...
- 自動檢查改成掃**所有**會 `DROP TABLE` 的遷移（不是只釘住已知那兩支），另外真的
...
- 自動檢查不釘工具名字（下一支新工具還是會漏），改成直接模擬那個升級情境：
  跑完整條遷移鏈之後，每一支註冊中的工具都要有角色拿得到。

### 修正 — 兩支工具搜尋不到

- 「送件前檢核」與「乘車證明整理」從加進來就沒有搜尋關鍵字。工具本身正常、選單裡
  也在，只有搜尋找不到 —— 所以不會有人回報。補上並加自動檢查（中英文都要有）。
...
- 送件檢核的跨案件儀表板住在工具路由底下，權限判斷寫成
  ...
  認證」。加了一支用 AST 比對的自動檢查，擋掉這種形狀再度出現。
...
- 兩者都不會有錯誤訊息，只會安靜地不動作。自動檢查改成比對 `<th data-col>` 與
...
- 自動檢查：API 一定要回這個欄位，且**模板讀的每個 `d.<欄位>` 都要真的在回應裡**。
...
- **安全**：client secret / SP 私鑰以 Fernet 加密存（金鑰沿用 session secret，檔案 mode 600，admin UI 只顯示遮罩值）；discovery/token/JWKS 端點限 http/https 並封鎖 cloud metadata 位址；SSO 端點納入 auth gate 公開白名單但啟用檢查（未啟用即回錯誤頁）。
