...
### 12. 文件翻譯 (`doc-translate`) — text

**LLM 做什麼**：
- 把辦公文件裡的每一段文字翻成目標語言，**產出同格式、同版面的檔案**（只換文字，不重排版面）
- 與逐句翻譯共用同一份 prompt：台灣繁中用語對照表、文件領域提示都一樣

**已實作**:
- 支援 `.doc` / `.docx` / `.odt`、`.xls` / `.xlsx` / `.ods`、`.ppt` / `.pptx` / `.odp`
- 以**段落**為單位送翻（不是逐個文字片段）—— 一句話常被切成好幾個片段，
  逐片段翻會把半句話丟給模型，譯文會走樣
- 單段翻譯失敗就保留原文，不讓整份作業掛掉
- 走背景作業，可以關掉分頁；翻完附前 6 頁預覽，用來確認版面沒跑掉
...
- **停用時一併隱藏**（預設不勾）：LLM 停用時，工具與選項預設照樣看得到、反灰，看得出為什麼不能用；
  勾了才從側欄、首頁與搜尋拿掉只靠 LLM 的工具，並藏起各工具的 LLM 選項。LLM 啟用時勾著不影響任何東西。
