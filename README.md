...
| | |
|---|---|
| 速度 | 28.3 分鐘的會議約 **112 秒**（比即時快 15 倍，實測） |
| 逐字稿 | 帶**時間與發言者**，可邊聽邊看：波形點一下就從那裡播、播到哪一段就亮起來 |
| 發言者 | 點代號直接改成人名，預設同一位全部一起改 |
| 準確度 | 拿公開語料（AISHELL-4）的標準答案量過，**整條管線與語音服務自己量的差 0.12 個百分點** |
