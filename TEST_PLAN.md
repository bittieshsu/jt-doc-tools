...
- [ ] 匯出 PDF 的「誰講了多少」表格：表頭底色**填滿整格**、框線淡、隔行淡底色、列高不會撐得很高（v1.16.19）
...
- [ ] **逐字稿改發言者名字，其他區塊跟著換**（v1.16.10）：待辦卡片的「負責：」、內文與摘要裡的
      `S1`、章節、心智圖、「誰講了多少」都換成新名字，重新整理與下載之後也是。
      `S12` 不可以被 `S1` 換掉一截；真人名字不做子字串取代；心智圖節點的 id 不可以跟著換
      （換了連線會斷）；檔名與「會議背景」不動。
- [ ] 兩個代號改成同一個名字 →「誰講了多少」合併成一位；只改某一段 → 那一段的次數與時間移過去，
      但卡片與摘要裡的代號不動。
...
- [ ] 表格在 PDF 上：表頭底色填滿整格（不是只包住字）、框線用主題的淡色、隔行上色；六種主題各看一次（v1.16.19）
