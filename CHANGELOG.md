...
* 順帶修正：「對方佇列滿了，N 秒後再試」「等了 N 分鐘…」這幾句伺服器送出的訊息，
  英文與日文介面一直顯示中文（語系檔裡沒有對應的譯文），現在都有了，並收進翻譯守門。
...
這幾句原本**一條測試都沒有**、也不在翻譯守門的範圍內（寫在錯誤碼對照表外面）——
...
「公開樹不可以有內網位址」這條守門掃的是整個目錄，而那個 job 用 `uv sync` 把虛擬環境
...
「比瀏覽器預設淡」補了守門（判準用亮度，不寫死色碼）。

> 守門第一版是空過的：管理頁那一條用 `input[placeholder]` 找欄位，對到的是
...
> 補了守門。第一版的修法在管理頁**量起來仍是整列寬** —— 真正蓋掉寬度的是另一條
> 權重更高的規則，是這支守門當場抓到的；它也抓到自己的一個空過（選項區在上傳前是
> 隱藏的，量到寬度 0）。
...
> 守門 `tests/test_docs_nav_controls_line_up.py` 在真的瀏覽器裡量
...
* `API.md` 收錄 `POST /tools/meeting-transcribe/api/meeting-transcribe`
  —— 原本的涵蓋守門是從「有 `/api/` 的工具」建表的，**一個 API 都沒有的
...
兩邊都修：端點改成 `api_route(methods=["GET", "HEAD"])`（有守門），
...
措辭收進一處，兩條路共用，守門驗「兩邊都委派過去、而且只有一份對照表」。
...
| Range 續傳 | 有（`FileResponse` 做的），但沒有守門 | 加了行為測試：206 ＋ `Content-Range` ＋ 位元組要對 |
...
> 我們會很有自信地講一個錯的數字。守門也跟著改成驗「說不說得出下一步」。
...
* **管理頁的守門靠「路徑尾段」比對** —— `/admin/jtlw` 一次都沒出現在測試計畫裡，
...
「推之前掃內網 IP」這條在慣例裡寫了很久，但**沒有守門** ——
記了規則沒有守門等於沒記，這次是推版前手動掃才發現的。
...
> **守門的判準不可以跟被守的程式共用同一個定義**：我第一版用產品自己的
...
> 猜出來的判準會在某些檔案上說謊。
>
> 守門**先證明那個前提成立**（純文字的結束時間真的是下一段的開始）——
...
> 已經吃過好幾次虧）。守門改走 AST，並且**專門有一條驗它不會把 `preview_url`
> 看成 `view_url`**。

> **守門的「先證明掃得到東西」當場抓到我自己**：第一版的 AST 比對只認
> `_jm.job_manager.submit`（`Attribute`），而多數工具是
> `from … import job_manager` 之後直接呼叫（`Name`）—— **29 支只掃到 3 支**。
> 沒有那條的話，這份守門會靜靜地只檢查三支工具，而且全綠。

> **判準要落在「我的作業」真的拿得到的資料上**：`view_url` 是在 `submit()`
> 之後才設的，跟清單之間隔著一次資料庫寫入。所以除了靜態守門，另有一條
...
> **既有的守門當場抓到我沒決定的事**：`test_all_workspace_office_types_are_covered`
...
已丟到背景執行緒。既有的「端點不可以鎖住事件迴圈」靜態守門看不到這種形狀
