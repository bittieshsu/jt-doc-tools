...
把 SignPath 筆記的擋法從「綁死一個目錄」改成「任何目錄都擋」時踩到：
我改的是 `github/.gitignore`，而**那個檔是 `sync-to-github.sh` 每次用
heredoc 重新產生的** —— 下一次同步就蓋回去，**而且完全無聲**（同步之前測試
是綠的，之後才紅）。規則活了不到一輪。

判準要落在**唯一來源**上：規則寫進同步腳本，守門另外驗「腳本裡有沒有那兩條」
（`test_the_rule_lives_where_it_actually_survives`）。只驗產出的話，
下一個人改錯地方一樣不會有人發現。

> 順手把擋法補強：原本只有 `packaging/windows/SIGNPATH-*.md`，
> **筆記換個資料夾就又會被公開** —— 跟當初「只列了 APPLICATION 那一檔」
> 是同一個病，只是換一個維度（上次漏的是檔名，這次會漏的是目錄）。
> 另外補一條守門直接掃公開樹的檔案系統：`git ls-files` **只看得到已經被
> 追蹤的**，一份剛放進去還沒 commit 的筆記在它眼裡是乾淨的，而
> `rsync -a --delete` 會把它帶進 clone、`git add -A` 就公開了。
