...
> ### ⚠ 安全提醒：不建議直接對「公開網際網路」開放
>
> 同仁多半會用它處理公司**內部 / 機密文件**（合約、報價、個資、統編資料等），直接對外
> 開放等於把這些文件與後台一併暴露，有**資料外洩風險**；加上本工具會解析上傳的 PDF /
> Office / 圖片（底層是 MuPDF / LibreOffice / Pillow 等記憶體不安全的原生程式，屬**高
> 風險攻擊面**）。
> - **首選：只在內網 / VPN 使用**（預設就只綁 `127.0.0.1`，安裝完直接本機用最安全）。
> - 內網多人用：一律走 **nginx / Caddy 反向代理 + HTTPS**，並啟用認證 + 強制 2FA。
> - **若因業務必須對外**屬「風險自負」，除上述外還要加 **WAF / 速率限制**、限縮上傳大小、
>   持續更新相依套件。詳見 [OPS.md](OPS.md)。
