...
#: 那是 mention 不是 use，跟「這一行忘了翻」是兩回事；不分開的話這條檢查會在
...
#      而既有檢查只驗「有沒有殘留中文」—— 錯位之後一個中文字都沒有，全綠。
#
...
#
# 答案是：**沒有檢查。** 上面那幾條只驗「檔案在不在、有沒有殘留中文、連結指不指
# 得回去」—— 一份停在 34 個版本以前的英文更新記錄，那三條**全部都是綠的**。
# 這個專案反覆出現同一句教訓：記了規則但沒有檢查，等於沒記。
#
# 判準要能自己算，不可以寫死期望值：
...
def test_japanese_pages_do_not_leave_a_stray_space_around_inline_tags():
    ...
    產生器 `_tighten_cjk_spaces()` 負責收掉。這一條是它的檢查：
