...
## 0.6 英文介面 —— **用真的瀏覽器逐頁掃**（使用者要求，2026-09-05）

切成英文之後畫面上還有中文，是**完全無聲**的：功能正常、測試全綠、也沒有任何
錯誤訊息，只有真的把介面切過去看的人才發現。靜態掃描只看得到模板裡沒包 `tr()`
的字串，看不到 ①JS 動態產生的節點 ②`title` / `placeholder` / `aria-label`
這些屬性 ③伺服器回傳的資料被塞進 DOM —— 而這三類佔了六成以上。

    # 1) 起一個 auth-off 的拋棄式實例（同 §1.8）
    JTDT_DATA_DIR=$(mktemp -d) JTDT_CSRF_DISABLE=1 uvicorn app.main:app --port 8799
    # 2) 逐頁掃（設 jtdt_locale=en cookie，取可見文字節點與屬性）
    python tools/i18n_untranslated_scan.py --base http://127.0.0.1:8799

- [ ] 掃描結果是 **0 條**（2026-09-05 的基準；有殘留就是漏包或漏翻）
- [ ] 中文畫面**位元組完全相同**：`python tools/i18n_zh_baseline.py --compare`
      （這是最高宗旨 —— i18n 不可以動到繁體中文的任何一個位元組）
- [ ] 英文相鄰標記的空白：`python tools/i18n_inline_gap_scan.py` 回報 0 條
      （`{{ tr('SSO 是') }}<b>{{ tr('附加') }}</b>` 會黏成 anadditionalway）
- [ ] 英文版介紹站用的是**英文介面的截圖**：`python tools/capture_en_screenshots.py`

**不該翻的不可以翻**：品牌名、語言選項本身（「繁體中文」）、欄位標籤同義詞
字典、統編資料庫的公司名、會計科目規則的關鍵字、頁碼格式（`第 {n} 頁` 會原樣
印進 PDF）、格式預覽的範例、要複製去貼的組態檔範例 —— 這些顯示中文是**正確的**，
翻掉功能會安靜失效。標記方式是在容器上加 `data-i18n="skip"`，掃描時跳過。
