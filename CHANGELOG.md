...
- **名單本身有順序**（第 1 位最優先），在設定面板拖曳「⠿」調整。沒有這個的話插隊
  只有一級，董事長跟部門主管會互相卡（先送出的先跑），管理員排的順序等於沒作用。
  跨排名照排名、同一位使用者的多件作業之間照先來後到 —— 用「排名同等或更前面才
  往後讓」而不是「更前面才讓」，否則後送出的那件會跑到自己先送出的那件前面。
- **改順序不會打亂已經在排隊的作業。** 排名是送出當下記在作業上的；每次派送都重查
  名單的話，管理員拖一下順序，別人已經排好的號碼就會往後跳。
- 作業清單的**送出者欄位**會標出他是名單第幾位（橘色排名徽章），「我的作業」也會
  顯示「優先派送（第 N 順位）」。這個標示看的是**目前**的名單，與作業送出當下的
  排隊結果分開 —— 名單改了不該讓歷史作業的排隊結果看起來變了。
- **記憶體准入仍然優先。** 插隊只改順序，不改「記憶體不夠就排隊」那條鐵則；否則一個
  主管的大檔轉檔就能把機器打爆，其他人連排隊的機會都沒有。
- 身分只由伺服器端的作業擁有者判斷，不看任何請求參數 —— 讓前端傳得動就等於開放
  所有人插隊。名單人數設上限（50 位）：名單一長就等於沒有優先順序可言。
